...
int main(int argc, char **argv) {
  ...
  /* fprintf(stdout, "# [hvp_caa_lma] Reading input from file %s\n", filename); */
  read_input_parser(filename);

#ifdef HAVE_TMLQCD_LIBWRAPPER

  fprintf(stdout, "# [hvp_caa_lma] calling tmLQCD wrapper init functions\n");
   ...
   *********************************/
#ifdef HAVE_OPENMP
  if(g_cart_id == 0) fprintf(stdout, "# [hvp_caa_lma] setting omp number of threads to %d\n", g_num_threads);
  omp_set_num_threads(g_num_threads);
#pragma omp parallel
{
  fprintf(stdout, "# [hvp_caa_lma] proc%.4d thread%.4d using %d threads\n", g_cart_id, omp_get_thread_num(), omp_get_num_threads());
}
#else
  if(g_cart_id == 0) fprintf(stdout, "[hvp_caa_lma] Warning, resetting global thread number to 1\n");
  g_num_threads = 1;
#endif

  if(init_geometry() != 0) {
    fprintf(stderr, "[hvp_caa_lma] Error from init_geometry %s %d\n", __FILE__, __LINE__);
    EXIT(4);
  }
  ...
  if(!(strcmp(gaugefilename_prefix,"identity")==0)) {
    ...
    if(g_cart_id==0) fprintf(stdout, "# [hvp_caa_lma] reading gauge field from file %s\n", filename);
    read_lime_gauge_field_doubleprec(filename);
  } else {
    /* initialize unit matrices */
    if(g_cart_id==0) fprintf(stdout, "\n# [hvp_caa_lma] initializing unit matrices\n");
  ...
  if(g_cart_id== 0) fprintf(stdout, "[hvp_caa_lma] Nconf = %d\n", Nconf);
  ...
  if( g_gauge_field == NULL) {
    fprintf(stderr, "[hvp_caa_lma] Error, g_gauge_field is NULL %s %d\n", __FILE__, __LINE__);
    EXIT(7);
  }
#endif
  ...
  if( exitstatus > 0) {
    fprintf(stderr, "[hvp_caa_lma] Error from tmLQCD_init_deflator, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(8);
  }

  exitstatus = tmLQCD_get_deflator_params(&g_tmLQCD_defl, _OP_ID_UP);
  if(exitstatus != 0) {
    fprintf(stderr, "[hvp_caa_lma] Error from tmLQCD_get_deflator_params, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(9);
  }

  if(g_cart_id == 1) {
    fprintf(stdout, "# [hvp_caa_lma] deflator type name = %s\n", g_tmLQCD_defl.type_name);
    fprintf(stdout, "# [hvp_caa_lma] deflator eo prec   = %d\n", g_tmLQCD_defl.eoprec);
    fprintf(stdout, "# [hvp_caa_lma] deflator precision = %d\n", g_tmLQCD_defl.prec);
    fprintf(stdout, "# [hvp_caa_lma] deflator nev       = %d\n", g_tmLQCD_defl.nev);
  }

  eo_evecs_block = (double*)(g_tmLQCD_defl.evecs);
  if(eo_evecs_block == NULL) {
    fprintf(stderr, "[hvp_caa_lma] Error, eo_evecs_block is NULL %s %d\n", __FILE__, __LINE__);
    EXIT(10);
  }

  evecs_num = g_tmLQCD_defl.nev;
  if(evecs_num == 0) {
    fprintf(stderr, "[hvp_caa_lma] Error, dimension of eigenspace is zero %s %d\n", __FILE__, __LINE__);
    EXIT(11);
  }
  ...
  if ( exitstatus != 0) {
    fprintf(stderr, "[hvp_caa_lma] Error from init_2level_buffer, status was %d\n", exitstatus);
    EXIT(1);
  }
  ...
  if(exitstatus != 0) {
    fprintf(stderr, "[hvp_caa_lma] Error from gauge_field_eq_gauge_field_ti_phase, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(38);
  }
  ...
  if( g_proc_coords[0] == 0 && g_proc_coords[1] == 0 && g_proc_coords[2] == 0 && g_proc_coords[3] == 0) {
    io_proc = 2;
    fprintf(stdout, "# [hvp_caa_lma] proc%.4d is io process\n", g_cart_id);
  } else {
    if( g_proc_coords[1] == 0 && g_proc_coords[2] == 0 && g_proc_coords[3] == 0) {
      io_proc = 1;
      fprintf(stdout, "# [hvp_caa_lma] proc%.4d is send process\n", g_cart_id);
    } else {
      io_proc = 0;
    }
  }
  ...
  if(io_proc == 2) {
    if(g_tr_id != 0) {
      fprintf(stderr, "[hvp_caa_lma] Error, io proc must be id 0 in g_tr_comm %s %d\n", __FILE__, __LINE__);
      EXIT(14);
    }
  }
#endif
  ...
  for( isource_location=0; isource_location < 1; isource_location++)
  {
    ...
    if( exitstatus != 0 ) {
      fprintf(stderr, "[hvp_caa_lma] Error from get_point_source_info status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
      EXIT(123);
    }
    ...
    if(exitstatus != 0 ) {
      fprintf(stderr, "[hvp_caa_lma] Error from cvc_tensor_tp_write_to_aff_file, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
      EXIT(45);
    }
    ...
    if( exitstatus != 0 ) {
      fprintf(stderr, "[hvp_caa_lma] Error from contract_local_cvc_2pt_eo, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
      EXIT(1);
    }
    ...
    if ( exitstatus != 0 ) {
      fprintf(stderr, "[hvp_caa_lma] Error from Q_clover_eo_invert_subspace, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
      EXIT(28);
    }
  ...
  if(g_cart_id==0) {
    g_the_time = time(NULL);
    fprintf(stdout, "# [hvp_caa_lma] %s# [hvp_caa_lma] end of run\n", ctime(&g_the_time));
    fprintf(stderr, "# [hvp_caa_lma] %s# [hvp_caa_lma] end of run\n", ctime(&g_the_time));
  }
