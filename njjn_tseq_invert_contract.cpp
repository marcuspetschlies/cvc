 ...
 ***************************************************************************/
void usage() {
  fprintf(stdout, "Code for TSEQ-type nucleon-nucleon 2-pt function inversion and contractions\n");
...
int main(int argc, char **argv) {
  ...
  */
  ...
  fprintf(stdout, "# [njjn_tseq_invert_contract] calling tmLQCD wrapper init functions\n");
   ...
   ***************************************************************************/
  if ( g_cart_id == 0 ) {
    fprintf(stdout, "# [njjn_tseq_invert_contract] git version = %s\n", g_gitversion);
  }
   ...
   ***************************************************************************/
#ifdef HAVE_OPENMP
  if(g_cart_id == 0) fprintf(stdout, "# [njjn_tseq_invert_contract] setting omp number of threads to %d\n", g_num_threads);
  omp_set_num_threads(g_num_threads);
#pragma omp parallel
{
  fprintf(stdout, "# [njjn_tseq_invert_contract] proc%.4d thread%.4d using %d threads\n", g_cart_id, omp_get_thread_num(), omp_get_num_threads());
}
#else
  if(g_cart_id == 0) fprintf(stdout, "[njjn_tseq_invert_contract] Warning, resetting global thread number to 1\n");
  g_num_threads = 1;
#endif

  if ( init_geometry() != 0 ) {
    fprintf(stderr, "[njjn_tseq_invert_contract] Error from init_geometry %s %d\n", __FILE__, __LINE__);
    EXIT(4);
  }
  ...
  if(!(strcmp(gaugefilename_prefix,"identity")==0)) {
    ...
    if(g_cart_id==0) fprintf(stdout, "# [njjn_tseq_invert_contract] reading gauge field from file %s\n", filename);
    exitstatus = read_lime_gauge_field_doubleprec(filename);
  } else {
    /* initialize unit matrices */
    if(g_cart_id==0) fprintf(stdout, "\n# [njjn_tseq_invert_contract] initializing unit matrices\n");
    exitstatus = unit_gauge_field ( g_gauge_field, VOLUME );
  }
#else
  Nconf = g_tmLQCD_lat.nstore;
  if(g_cart_id== 0) fprintf(stdout, "[njjn_tseq_invert_contract] Nconf = %d\n", Nconf);
  ...
  if( g_gauge_field == NULL) {
    fprintf(stderr, "[njjn_tseq_invert_contract] Error, g_gauge_field is NULL %s %d\n", __FILE__, __LINE__);
    EXIT(7);
  }
#endif
  ...
  if(exitstatus != 0) {
    fprintf(stderr, "[njjn_tseq_invert_contract] Error from gauge_field_eq_gauge_field_ti_phase, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(38);
  }
  ...
  if ( exitstatus != 0 ) {
    fprintf(stderr, "[njjn_tseq_invert_contract] Error from init_clover, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
    EXIT(1);
  }
  ...
  io_proc = get_io_proc ();
  if( io_proc < 0 ) {
    fprintf(stderr, "[njjn_tseq_invert_contract] Error, io proc must be ge 0 %s %d\n", __FILE__, __LINE__);
    EXIT(14);
  }
  fprintf(stdout, "# [njjn_tseq_invert_contract] proc%.4d has io proc id %d\n", g_cart_id, io_proc );
  ...
  if ( exitstatus != 0 ) {
    fprintf(stderr, "[njjn_tseq_invert_contract] Error from init_rng_stat_file %s %d\n", __FILE__, __LINE__ );;
    EXIT( 50 );
  }
  ...
  if ( loop  == NULL ) {
    fprintf ( stderr, "[njjn_tseq_invert_contract] Error from init_Xlevel_ztable %s %d\n", __FILE__, __LINE__ );
    EXIT(12);
  }
  ...
  if ( io_proc == 2 && g_verbose > 0 ) {
    fprintf ( stdout, "# [njjn_tseq_invert_contract] reading loop field from file %s %s %d\n", filename,  __FILE__, __LINE__ );
  }

  exitstatus = read_lime_contraction ( (double*)(loop[0][0]), filename, 144, 0 );
  if ( exitstatus != 0  ) {
    fprintf ( stderr, "[njjn_tseq_invert_contract] Error read_lime_contraction, status was %d  %s %d\n", exitstatus, __FILE__, __LINE__ );
    EXIT(12);
  }

#endif  /* of if _USE_LOOP */
  ...
  if ( spinor_work == NULL ) {
    fprintf ( stderr, "[njjn_tseq_invert_contract] Error from init_Xlevel_dtable %s %d\n", __FILE__, __LINE__ );
    EXIT(12);
  }
...
#  endif
  {
    fprintf(stderr, "[njjn_tseq_invert_contract] Error from invert, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(12);
  }
  ...
  if( N_Jacobi > 0 ) {
    ...
    if ( N_ape > 0 ) {
      ...
      if(exitstatus != 0) {
        fprintf(stderr, "[njjn_tseq_invert_contract] Error from APE_Smearing, status was %d\n", exitstatus);
        EXIT(47);
      }
#ifndef _SMEAR_QUDA 
    }  /* end of if N_ape > 0 */
  ...
  for( int isource_location = 0; isource_location < g_source_location_number; isource_location++ ) {
    ...
    if( exitstatus != 0 ) {
      fprintf(stderr, "[njjn_tseq_invert_contract] Error from get_point_source_info status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
      EXIT(123);
    }
     ...
     ***************************************************************************/
      ...
      fprintf(stdout, "# [njjn_tseq_invert_contract] writing data to file %s\n", filename);
      affw = aff_writer(filename);
      const char * aff_status_str = aff_writer_errstr ( affw );
      if( aff_status_str != NULL ) {
        fprintf(stderr, "[njjn_tseq_invert_contract] Error from aff_writer, status was %s %s %d\n", aff_status_str, __FILE__, __LINE__);
        EXIT(15);
      }
#else
      fprintf(stderr, "[njjn_tseq_invert_contract] Error, no outupt variant selected %s %d\n",  __FILE__, __LINE__);
      EXIT(15);
#endif
    }  /* end of if io_proc == 2 */
     ...
     ** Part IIa
     **
     ** point-to-all propagators with source at source
     **
     ***************************************************************************
     ***************************************************************************/
      ...
      for ( int iflavor = 0; iflavor < 2; iflavor++ ) 
      {
        ...
        if(exitstatus != 0) {
          fprintf(stderr, "[njjn_tseq_invert_contract] Error from point_source_propagator, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
          EXIT(12);
        }
      
        gettimeofday ( &tb, (struct timezone *)NULL );
        show_time ( &ta, &tb, "njjn_tseq_invert_contract", "forward-light-smear-invert-check", g_cart_id == 0 );
        ...
        for ( int i = 0; i < 12; i++ ) {
          ...
          if(exitstatus != 0) {
            fprintf(stderr, "[njjn_tseq_invert_contract] Error from Jacobi_Smearing, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
            return(11);
          }
        }
      
        gettimeofday ( &tb, (struct timezone *)NULL );
        show_time ( &ta, &tb, "njjn_tseq_invert_contract", "forward-light-sink-smear", g_cart_id == 0 );
        ...
        if ( g_write_propagator ) {
          ...
          for ( int i = 0; i < 12; i++ ) {
            ...
            if ( ( exitstatus = write_propagator( propagator[iflavor][i], filename, 0, g_propagator_precision) ) != 0 ) {
              fprintf(stderr, "[njjn_tseq_invert_contract] Error from write_propagator, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
              EXIT(2);
            }
          }
        }
      ...
      for ( int iflavor = 0; iflavor < 2; iflavor++ ) {
        ...
        if ( vx == NULL ) {
          fprintf(stderr, "[njjn_tseq_invert_contract] Error from init_2level_dtable, %s %d\n", __FILE__, __LINE__);
          EXIT(47);
        }
        ...
        if ( vp == NULL ) {
          fprintf(stderr, "[njjn_tseq_invert_contract] Error from init_3level_dtable %s %d\n", __FILE__, __LINE__ );
          EXIT(47);
        }
        ...
        for ( int if2 = 0; if2 < gamma_f1_number; if2++ ) {
          ...
          if ( exitstatus != 0 ) {
            fprintf(stderr, "[njjn_tseq_invert_contract] Error from reduce_project_write, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
            EXIT(48);
          }
        ...
        gettimeofday ( &tb, (struct timezone *)NULL );
        show_time ( &ta, &tb, "njjn_tseq_invert_contract", "n1-n2-reduce-project-write", g_cart_id == 0 );
     ...
     ***************************************************************************/
      ...
      for ( int iflavor = 0; iflavor < 2; iflavor++ )
      {
        ...
        if ( g_cart_id == 0 && g_verbose > 2 ) fprintf(stdout, "# [njjn_tseq_invert_contract] start seq source flavor %3d   %s %d\n", 
        ...
        for ( int seq_source_type = 0; seq_source_type <= 1; seq_source_type++ )
        {
          ...
          if ( g_cart_id == 0 && g_verbose > 2 ) fprintf(stdout, "# [njjn_tseq_invert_contract] start seq source type %3d   %s %d\n", 
          ...
          for ( int igamma = 0; igamma < sequential_gamma_sets; igamma++ ) {
            ...
            if ( g_cart_id == 0 && g_verbose > 2 ) fprintf(stdout, "# [njjn_tseq_invert_contract] start seq source gamma set %s   %s %d\n", 
            ...
            for ( int iloop_flavor = 0; iloop_flavor < ( seq_source_type == 0 ? 2 : 1 ); iloop_flavor++ ) {
              ...
              if ( g_cart_id == 0 && g_verbose > 2 ) fprintf(stdout, "# [njjn_tseq_invert_contract] using flavor %d / loop_flavor %d for seq source type %d  %s %d\n", 
              ...
              if ( exitstatus != 0 ) {
                fprintf ( stderr, "[njjn_tseq_invert_contract] Error from prepare_sequential_fht_loop_source, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
                EXIT(123);
              }
              ...
              for ( int itseq = 0; itseq < g_sequential_source_timeslice_number; itseq++ ) 
              {
                ...
                int const gtseq = ( tseq + gsx[0] + T_global ) % T_global;
                int const have_tseq = ( gtseq / T ==  g_proc_coords[0] );
                int const ltseq = gtseq % T;

                double ** sequential_timeslice_source = init_2level_dtable ( 12, _GSI(VOLUME) );
                if ( sequential_timeslice_source == NULL ) {
                  fprintf(stderr, "[njjn_tseq_invert_contract] Error from  init_2level_dtable %s %d\n", __FILE__, __LINE__);
                  EXIT(2);
                }

                memset ( sequential_timeslice_source[0], 0, 12*sizeof_spinor_field );
                if ( have_tseq ) {
                  size_t const offset = _GSI ( ltseq * VOL3 );
                  for ( int i = 0; i < 12; i++ ) {
                    memcpy ( sequential_timeslice_source[i]+offset, sequential_source[i]+offset, sizeof_spinor_field_timeslice );
                  }
                }

                if ( g_write_sequential_source ) {
                  for ( int i = 0; i < 12; i++ ) {
                    sprintf ( filename, "sequential_source_%c.%.4d.t%dx%dy%dz%d.%s.type%d.%d.%.5d.tseq%d",
                        flavor_tag[iflavor], Nconf, gsx[0], gsx[1], gsx[2], gsx[3],
                        sequential_gamma_tag[igamma], seq_source_type, i, g_nsample_oet, tseq );

                    if ( ( exitstatus = write_propagator( sequential_timeslice_source[i], filename, 0, g_propagator_precision) ) != 0 ) {
                      fprintf(stderr, "[njjn_tseq_invert_contract] Error from write_propagator, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
                      EXIT(2);
                    }
                  }
                ...
                gettimeofday ( &tb, (struct timezone *)NULL );
                show_time ( &ta, &tb, "njjn_tseq_invert_contract", "prepare-sequential-fht-source", g_cart_id == 0 );
	        ...
	        gettimeofday ( &ta, (struct timezone *)NULL );
                ...
                exitstatus = prepare_propagator_from_source ( sequential_propagator, sequential_timeslice_source, 12, iflavor, 0, 1, gauge_field_smeared,
                    check_propagator_residual, gauge_field_with_phase, lmzz, NULL );
                if ( exitstatus != 0 ) {
                  fprintf ( stderr, "[njjn_tseq_invert_contract] Error from prepare_propagator_from_source, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
                  EXIT(123);
                }

                if ( g_write_sequential_propagator ) {
                  for ( int i = 0; i < 12; i++ ) {
                    sprintf ( filename, "sequential_source_%c.%.4d.t%dx%dy%dz%d.%s.type%d.%d.%.5d.tseq%d.inverted",
                        flavor_tag[iflavor], Nconf, gsx[0], gsx[1], gsx[2], gsx[3],
                        sequential_gamma_tag[igamma], seq_source_type, i, g_nsample_oet , tseq );
                ...
                gettimeofday ( &tb, (struct timezone *)NULL );
                show_time ( &ta, &tb, "njjn_tseq_invert_contract", "sequential-source-invert-check-smear", g_cart_id == 0 );

                fini_2level_dtable ( &sequential_timeslice_source );
                ...
                sprintf ( aff_tag_prefix, "/%s/%c%c%c%c-f%c-f%c/nsample%d/Gc_%s/tseq%d",
                        correlator_tag, 
                        sequential_propagator_name, flavor_tag[iflavor], flavor_tag[loop_flavor], flavor_tag[iflavor],
                        flavor_tag[1-iflavor],
                        flavor_tag[iflavor],
                        g_nsample,
                        sequential_gamma_tag[igamma], tseq );

                sprintf ( aff_tag_prefix2, "/%s/f%c-f%c-%c%c%c%c/nsample%d/Gc_%s/tseq%d",
                        correlator_tag,
                        flavor_tag[iflavor],
                        flavor_tag[1-iflavor],
                        sequential_propagator_name, flavor_tag[iflavor], flavor_tag[loop_flavor], flavor_tag[iflavor],
  	        ...
  	        gettimeofday ( &ta, (struct timezone *)NULL );
                ...
                gettimeofday ( &tb, (struct timezone *)NULL );
                show_time ( &ta, &tb, "njjn_tseq_invert_contract", "buuu-duuu-reduce-project-write", g_cart_id == 0 );
                ...
                sprintf ( aff_tag_prefix, "/%s/f%c-%c%c%c%c-f%c/nsample%d/Gc_%s/tseq%d",
                        correlator_tag,
                        flavor_tag[1-iflavor], 
                        sequential_propagator_name, flavor_tag[iflavor], flavor_tag[loop_flavor], flavor_tag[iflavor],
                        flavor_tag[1-iflavor],
                ...
                gettimeofday ( &tb, (struct timezone *)NULL );
                show_time ( &ta, &tb, "njjn_tseq_invert_contract", "bddd-dddd-reduce-project-write", g_cart_id == 0 );
    ...
    if(io_proc == 2) {
      ...
      if( aff_status_str != NULL ) {
        fprintf(stderr, "[njjn_tseq_invert_contract] Error from aff_writer_close, status was %s %s %d\n", aff_status_str, __FILE__, __LINE__);
        EXIT(32);
      }
    }  /* end of if io_proc == 2 */
  ...
  ***************************************************************************/
  ...
  show_time ( &start_time, &end_time, "njjn_tseq_invert_contract", "runtime", g_cart_id == 0 );
