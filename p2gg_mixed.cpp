  ...
  for(isource_location=0; isource_location < g_source_location_number; isource_location++) {
    ...
    {  /* loop on shifts in direction mu */
      ...
      for(i=0; i<12; i++) 
      {
        ...
        int eo_spinor_field_id_o = 60 + eo_spinor_field_id_e;
        ...
        if(exitstatus != 0 ) {
          fprintf(stderr, "[p2gg_mixed] Error from init_eo_spincolor_pointsource_propagator; status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
          EXIT(18);
        }

        /* C^-1 */
        if(g_cart_id == 0) fprintf(stdout, "# [p2gg_mixed] calling _TMLQCD_INVERT_EO\n");
        memset(eo_spinor_work[1], 0, sizeof_eo_spinor_field);
        ...
        if(exitstatus != 0) {
          fprintf(stderr, "[p2gg_mixed] Error from fini_eo_propagator, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
          EXIT(20);
        }
