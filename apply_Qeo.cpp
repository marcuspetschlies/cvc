...
*/
  ...
  if(g_cart_id==0) fprintf(stdout, "# [apply_Qeo] Reading input from file %s\n", filename);
  ...
  if(g_cart_id==0) fprintf(stdout, "# [apply_Qeo] reading gauge field from file %s\n", filename);
  ...
  if(status != 0) {
    fprintf(stderr, "[apply_Qeo] Error, could not read gauge field\n");
    EXIT(11);
  }
  xchange_gauge();

  /* measure the plaquette */
  if(g_cart_id==0) fprintf(stdout, "# [apply_Qeo] read plaquette value 1st field: %25.16e\n", plaq);
  plaquette(&plaq);
  if(g_cart_id==0) fprintf(stdout, "# [apply_Qeo] measured plaquette value 1st field: %25.16e\n", plaq);
  ...
  if(g_cart_id == 0) fprintf(stdout, "# [apply_Qeo] reading C_oo_sym eigenvector from file %s\n", filename);

  status = read_lime_spinor(g_spinor_field[0], filename, 0);
  if( status != 0) {
    fprintf(stderr, "[apply_Qeo] Error from read_lime_spinor, status was %d\n");
    EXIT(1);
  }
  ...
  if(g_cart_id == 0) fprintf(stdout, "# [apply_Qeo] global source coordinates = (%d, %d, %d, %d)\n", gsx0, gsx1, gsx2, gsx3);
  ...
  if(source_proc_id == g_cart_id) {
    fprintf(stdout, "# [apply_Qeo] process %d has the source at %d with local coordinates = (%d, %d, %d, %d)\n", source_proc_id, l_source_location, lsx0, lsx1, lsx2, lsx3);
  }
  ...
  if(source_proc_id == g_cart_id) {
    ...
    fprintf(stdout, "# [apply_Qeo] g_lexic2eosub = %d, g_lexic2eo = %d, VOUME = %d, RAND = %d\n", g_lexic2eosub[ix], g_lexic2eo[ix], VOLUME, RAND);
    if(g_iseven[ix]) {
  ...
  if(g_cart_id == 0) fprintf(stdout, "# [apply_Qeo] eo norm = %e; norm2 = %e, sum = %e\n", sqrt(norm), sqrt(norm2), sqrt(norm + norm2));
...
*/
  ...
  if (g_cart_id == 0) {
    ...
    fprintf(stdout, "# [apply_Qeo] %s# [apply_Qeo] end fo run\n", ctime(&g_the_time));
    fflush(stdout);
    fprintf(stderr, "# [apply_Qeo] %s# [apply_Qeo] end fo run\n", ctime(&g_the_time));
    fflush(stderr);
  }
