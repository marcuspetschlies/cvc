...
unsigned long long local_to_global_site_index(const int local_site_index)
{
  return( (unsigned long long)local_site_index +
          ...
          g_nproc_z*VOLUME*g_proc_coords[DIM_Y] +
          g_nproc_y*g_nproc_z*VOLUME*g_proc_coords[DIM_X] +
          g_nproc_x*g_nproc_y*g_nproc_z*VOLUME*g_proc_coords[DIM_T] );
}
...
static inline bool is_local(unsigned long long global_site_index)
{
  ...
  proc_z = global_site_index % g_nproc_z;
  idx /= g_nproc_z;
