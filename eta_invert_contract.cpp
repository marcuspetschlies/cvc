...
void usage() {
  fprintf(stdout, "Code to calculate eta/eta' system 2-pt function\n");
...
int main(int argc, char **argv) {
  ...
  /* fprintf(stdout, "# [eta_invert_contract] Reading input from file %s\n", filename); */
  read_input_parser(filename);

#ifdef HAVE_TMLQCD_LIBWRAPPER

  fprintf(stdout, "# [eta_invert_contract] calling tmLQCD wrapper init functions\n");
   ...
   ***************************************************************************/
  if ( g_cart_id == 0 ) {
    fprintf(stdout, "# [eta_invert_contract] git version = %s\n", g_gitversion);
  }
   ...
   ***************************************************************************/
  if ( init_geometry() != 0 ) {
    fprintf(stderr, "[eta_invert_contract] Error from init_geometry %s %d\n", __FILE__, __LINE__);
    EXIT(4);
  }
  ...
  if(!(strcmp(gaugefilename_prefix,"identity")==0)) {
    ...
    if(g_cart_id==0) fprintf(stdout, "# [eta_invert_contract] reading gauge field from file %s\n", filename);
    exitstatus = read_lime_gauge_field_doubleprec(filename);
  } else {
    /* initialize unit matrices */
    if(g_cart_id==0) fprintf(stdout, "\n# [eta_invert_contract] initializing unit matrices\n");
    exitstatus = unit_gauge_field ( g_gauge_field, VOLUME );
  }
  if(exitstatus != 0) {
    fprintf ( stderr, "[eta_invert_contract] Error initializing gauge field, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
    EXIT(8);
  }
#else
  Nconf = g_tmLQCD_lat.nstore;
  if(g_cart_id== 0) fprintf(stdout, "[eta_invert_contract] Nconf = %d\n", Nconf);
  ...
  if( g_gauge_field == NULL) {
    fprintf(stderr, "[eta_invert_contract] Error, g_gauge_field is NULL %s %d\n", __FILE__, __LINE__);
    EXIT(7);
  }
#endif
  ...
  if(exitstatus != 0) {
    fprintf(stderr, "[eta_invert_contract] Error from gauge_field_eq_gauge_field_ti_phase, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(38);
  }
  ...
  if(exitstatus != 0) {
    fprintf(stderr, "[eta_invert_contract] Error from plaquetteria, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(38);
  }
  ...
  if ( exitstatus != 0 ) {
    fprintf(stderr, "[eta_invert_contract] Error from init_clover, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
    EXIT(1);
  }
  ...
  io_proc = get_io_proc ();
  if( io_proc < 0 ) {
    fprintf(stderr, "[eta_invert_contract] Error, io proc must be ge 0 %s %d\n", __FILE__, __LINE__);
    EXIT(14);
  }
  fprintf(stdout, "# [eta_invert_contract] proc%.4d has io proc id %d\n", g_cart_id, io_proc );
  ...
  if ( spinor_work == NULL ) {
    fprintf(stderr, "[eta_invert_contract] Error from init_2level_dtable %s %d\n", __FILE__, __LINE__ );
    EXIT(1);
  }
  ...
  if ( stochastic_propagator_mom_smeared_list == NULL ) {
    fprintf(stderr, "[eta_invert_contract] Error from init_Xlevel_dtable %s %d\n", __FILE__, __LINE__ );
    EXIT(48);
  }

  /***************************************************************************/

  double *** stochastic_propagator_zero_smeared_list = init_3level_dtable ( 2, spin_color_dilution, nelem );
...
#  endif
  ...
  if ( rng_status_read ) {
    ...
    if ( exitstatus != 0 ) {
      fprintf(stderr, "[eta_invert_contract] Error from read_rng_state %s %d\n", __FILE__, __LINE__ );;
      EXIT( 50 );
    }
    ...
    if ( exitstatus != 0 ) {
      fprintf(stderr, "[eta_invert_contract] Error from init_rng_state %s %d\n", __FILE__, __LINE__ );;
      EXIT( 50 );
    }
  }
  ...
  for ( int isample = g_sourceid; isample <= g_sourceid2; isample += g_sourceid_step )
  {
    ...
    if ( g_cart_id == 0 ) fprintf ( stdout, "# [eta_invert_contract] gts = %d %s %d\n", gts, __FILE__, __LINE__ );

#ifdef HAVE_MPI
    if (  MPI_Bcast( &gts, 1, MPI_INT, 0, g_cart_grid ) != MPI_SUCCESS ) {
      fprintf ( stderr, "[eta_invert_contract] Error from MPI_Bcast %s %d\n", __FILE__, __LINE__ );
      EXIT(12);
    }
#endif
    ...
    if( exitstatus != 0 ) {
      fprintf(stderr, "[eta_invert_contract] Error from get_timeslice_source_info status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
      EXIT(123);
    }
    ...
    if(io_proc == 2) {
      ...
      if( aff_status_str != NULL ) {
        fprintf(stderr, "[eta_invert_contract] Error from aff_writer, status was %s %s %d\n", aff_status_str, __FILE__, __LINE__);
        EXIT(15);
      }
    }  /* end of if io_proc == 2 */
    ...
    if(io_proc == 2 && g_verbose > 1 ) { 
      fprintf(stdout, "# [eta_invert_contract] writing data to file %s\n", output_filename);
    }
    ...
    if(exitstatus != 0) {
      fprintf(stderr, "[eta_invert_contract] Error from sync_rng_state, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
      EXIT(38);
    }
    ...
    if( (exitstatus = init_timeslice_source_oet(stochastic_source_list, gts, NULL, spin_dilution, color_dilution, 1 ) ) != 0 ) {
      fprintf(stderr, "[eta_invert_contract] Error from init_timeslice_source_oet, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
      EXIT(64);
    }
    if ( g_write_source ) {
      for ( int i = 0; i < spin_color_dilution; i++ ) {
        ...
        if ( ( exitstatus = write_propagator( stochastic_source_list[i], filename, 0, g_propagator_precision) ) != 0 ) {
          fprintf(stderr, "[eta_invert_contract] Error from write_propagator, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
          EXIT(2);
        }
      }
    }
    ...
    if(exitstatus != 0) {
      fprintf(stderr, "[eta_invert_contract] Error from get_rng_state, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
      EXIT(38);
    }

    exitstatus = save_rng_state ( 0, NULL );
    if ( exitstatus != 0 ) {
      fprintf(stderr, "[eta_invert_contract] Error from save_rng_state, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );;
      EXIT(38);
    }
    ...
    for ( int ils = 0; ils < 2; ils++ ) 
    {
      ...
      for ( int ipi = 0; ipi < g_source_momentum_number; ipi++ )
      {
        ...
        if ( N_Jacobi > 0 ) {
          ...
          for( int i = 0; i < spin_color_dilution; i++) {
            ...
            if(exitstatus != 0) {
              fprintf(stderr, "[eta_invert_contract] Error from Jacobi_Smearing, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
              return(11);
            }
          }
        }
        ...
        for( int i = 0; i < spin_color_dilution; i++)
        {
          ...
          exitstatus = _TMLQCD_INVERT ( spinor_work[1], spinor_work[0], 2*ils );
          if(exitstatus < 0) {
            fprintf(stderr, "[eta_invert_contract] Error from invert, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
            EXIT(44);
          }
  
          if ( check_propagator_residual ) {
            check_residual_clover ( &(spinor_work[1]), &(spinor_work[0]), gauge_field_with_phase, (ils == 0 ? mzz[0] : smzz[0]), (ils == 0 ? mzzinv[0] : smzzinv[0]), 1 );
          }
      ...
      * 3-momentum vector at source set to zero
      ...
      /***************************************************************************
       * invert for flavor id up and dn
       ***************************************************************************/
      for ( int imu = 0; imu < 2; imu++ )
      {
        for( int i = 0; i < spin_color_dilution; i++) {
          ...
          exitstatus = _TMLQCD_INVERT ( spinor_work[1], spinor_work[0], 2*ils+imu );
          if(exitstatus < 0) {
            fprintf(stderr, "[eta_invert_contract] Error from invert, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
            EXIT(44);
          }
          ...
          if ( check_propagator_residual ) {
            check_residual_clover ( &(spinor_work[1]), &(spinor_work[0]), gauge_field_with_phase, (ils == 0 ? mzz[imu] : smzz[imu]), (ils == 0 ? mzzinv[imu] : smzzinv[imu]), 1 );
          }
   
          memcpy( stochastic_propagator_zero_smeared_list[imu][i], spinor_work[1], sizeof_spinor_field);
    
          /***************************************************************************
           * SINK SMEARING
           ***************************************************************************/
          if ( N_Jacobi > 0 ) {
            exitstatus = Jacobi_Smearing ( gauge_field_smeared, stochastic_propagator_zero_smeared_list[imu][i], N_Jacobi, kappa_Jacobi);
            if(exitstatus != 0) {
              fprintf(stderr, "[eta_invert_contract] Error from Jacobi_Smearing, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
              EXIT(11);
            }
      ...
      for ( int igi = 0; igi < g_source_gamma_id_number; igi++ )
      {
        ...
        for ( int igf = 0; igf < g_sink_gamma_id_number; igf++ )
        {
          ...
          for ( int ipi = 0; ipi < g_source_momentum_number; ipi++ ) {
            ...
            if  ( contr_x == NULL ) {
              fprintf(stderr, "[eta_invert_contract] Error from init_1level_dtable %s %d\n", __FILE__, __LINE__);
              EXIT(3);
            }
            ...
            contract_twopoint_xdep ( contr_x, source_gamma, sink_gamma, 
                stochastic_propagator_zero_smeared_list[1], 
            ...
            if (exitstatus != 0) {
              fprintf(stderr, "[eta_invert_contract] Error from momentum_projection, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
              EXIT(3);
            }
      
            sprintf ( data_tag, "/%s-gf-%s-gi/mu%6.4f/mu%6.4f/t%d/s%d/gf%d/gi%d/pix%dpiy%dpiz%d", flavor_tag[2*ils+0], flavor_tag[2*ils+0],
                muval[2*ils+0], muval[2*ils+0],
                gts, isample,
  ...
  #endif
            if(exitstatus != 0) {
              fprintf(stderr, "[eta_invert_contract] Error from contract_write_to_file, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
              EXIT(3);
            }
           ...
           sprintf ( data_tag, "/%s-gf-%s-gi/mu%6.4f/mu%6.4f/t%d/s%d/gf%d/gi%d/pix%dpiy%dpiz%d", flavor_tag[2*ils+1], flavor_tag[2*ils],
               muval[2*ils+1], muval[2*ils],
               gts, isample,
               sink_gamma, source_gamma,
               source_momentum[0], source_momentum[1], source_momentum[2] );
      ...
      }  /* end of loop gamma at source */
  
      /*****************************************************************/
      /*****************************************************************/
  
    }  /* loop on light, strange */

#if ( defined HAVE_LHPC_AFF ) && ! ( defined HAVE_HDF5 )
    if(io_proc == 2) {
      const char * aff_status_str = (char*)aff_writer_close (affw);
      if( aff_status_str != NULL ) {
        fprintf(stderr, "[eta_invert_contract] Error from aff_writer_close, status was %s %s %d\n", aff_status_str, __FILE__, __LINE__);
        EXIT(32);
      }
    }  /* end of if io_proc == 2 */
  ...
  if(g_cart_id==0) {
    g_the_time = time(NULL);
    fprintf(stdout, "# [eta_invert_contract] %s# [eta_invert_contract] end of run\n", ctime(&g_the_time));
    fprintf(stderr, "# [eta_invert_contract] %s# [eta_invert_contract] end of run\n", ctime(&g_the_time));
  }
