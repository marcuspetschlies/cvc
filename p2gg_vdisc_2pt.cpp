...
int main(int argc, char **argv) {
  ...
  /* fprintf(stdout, "# [p2gg_vdisc_2pt] Reading input from file %s\n", filename); */
  read_input_parser(filename);

#ifdef HAVE_TMLQCD_LIBWRAPPER

  fprintf(stdout, "# [p2gg_vdisc_2pt] calling tmLQCD wrapper init functions\n");
   ...
   ***************************************************************************/
  if ( g_cart_id == 0 ) {
    fprintf(stdout, "# [p2gg_vdisc_2pt] git version = %s\n", g_gitversion);
  }
   ...
   ***************************************************************************/
  if ( init_geometry() != 0 ) {
    fprintf(stderr, "[p2gg_vdisc_2pt] Error from init_geometry %s %d\n", __FILE__, __LINE__);
    EXIT(4);
  }
  ...
  if(!(strcmp(gaugefilename_prefix,"identity")==0)) {
    ...
    if(g_cart_id==0) fprintf(stdout, "# [p2gg_vdisc_2pt] reading gauge field from file %s\n", filename);
    exitstatus = read_lime_gauge_field_doubleprec(filename);
  } else {
    /* initialize unit matrices */
    if(g_cart_id==0) fprintf(stdout, "\n# [p2gg_vdisc_2pt] initializing unit matrices\n");
    exitstatus = unit_gauge_field ( g_gauge_field, VOLUME );
  }
  if(exitstatus != 0) {
    fprintf ( stderr, "[p2gg_vdisc_2pt] Error initializing gauge field, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
    EXIT(8);
  }
#else
  Nconf = g_tmLQCD_lat.nstore;
  if(g_cart_id== 0) fprintf(stdout, "[p2gg_vdisc_2pt] Nconf = %d\n", Nconf);
  ...
  if( g_gauge_field == NULL) {
    fprintf(stderr, "[p2gg_vdisc_2pt] Error, g_gauge_field is NULL %s %d\n", __FILE__, __LINE__);
    EXIT(7);
  }
#endif
  ...
  if(exitstatus != 0) {
    fprintf(stderr, "[p2gg_vdisc_2pt] Error from gauge_field_eq_gauge_field_ti_phase, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(38);
  }
  ...
  if(exitstatus != 0) {
    fprintf(stderr, "[p2gg_vdisc_2pt] Error from plaquetteria, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(38);
  }
  ...
  if ( exitstatus != 0 ) {
    fprintf(stderr, "[p2gg_vdisc_2pt] Error from init_clover, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
    EXIT(1);
  }
  ...
  io_proc = get_io_proc ();
  if( io_proc < 0 ) {
    fprintf(stderr, "[p2gg_vdisc_2pt] Error, io proc must be ge 0 %s %d\n", __FILE__, __LINE__);
    EXIT(14);
  }
  fprintf(stdout, "# [p2gg_vdisc_2pt] proc%.4d has io proc id %d\n", g_cart_id, io_proc );
  ...
  if ( spinor_work == NULL ) {
    fprintf(stderr, "[p2gg_vdisc_2pt] Error from init_2level_dtable %s %d\n", __FILE__, __LINE__ );
    EXIT(1);
  }
  ...
  if ( stochastic_propagator_mom_list == NULL ) {
    fprintf(stderr, "[p2gg_vdisc_2pt] Error from init_Xlevel_dtable %s %d\n", __FILE__, __LINE__ );
    EXIT(48);
  }
  ...
  if ( exitstatus != 0 ) {
    fprintf(stderr, "[p2gg_vdisc_2pt] Error from init_rng_state %s %d\n", __FILE__, __LINE__ );;
    EXIT( 50 );
  }
  ...
  if(io_proc == 2) {
    ...
    if( aff_status_str != NULL ) {
      fprintf(stderr, "[p2gg_vdisc_2pt] Error from aff_writer, status was %s %s %d\n", aff_status_str, __FILE__, __LINE__);
      EXIT(15);
    }
  }  /* end of if io_proc == 2 */
  ...
  if(io_proc == 2 && g_verbose > 1 ) { 
    fprintf(stdout, "# [p2gg_vdisc_2pt] writing data to file %s\n", output_filename);
  }
  ...
  for( int isource_location = 0; isource_location < T_global; isource_location++ )
  {
    ...
    if( exitstatus != 0 ) {
      fprintf(stderr, "[p2gg_vdisc_2pt] Error from get_timeslice_source_info status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
      EXIT(123);
    }
    ...
    for ( int isample = 0; isample < g_nsample_oet; isample++ ) {
      ...
      if(exitstatus != 0) {
        fprintf(stderr, "[p2gg_vdisc_2pt] Error from sync_rng_state, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
        EXIT(38);
      }
      ...
      if ( g_read_source ) {
        for ( int i = 0; i < spin_color_dilution; i++ ) {
          ...
          if ( ( exitstatus = read_lime_spinor( stochastic_source_list[i], filename, 0) ) != 0 ) {
            fprintf(stderr, "[p2gg_vdisc_2pt] Error from read_lime_spinor, status was %d\n", exitstatus);
            EXIT(2);
          }
        }
        ...
        if( exitstatus != 0 ) {
          fprintf(stderr, "[p2gg_vdisc_2pt] Error from init_timeslice_source_oet, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
          EXIT(64);
        }
      ...
      if(exitstatus != 0) {
        fprintf(stderr, "[p2gg_vdisc_2pt] Error from get_rng_state, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
        EXIT(38);
      }

      exitstatus = save_rng_state ( 0, NULL );
      if ( exitstatus != 0 ) {
        fprintf(stderr, "[p2gg_vdisc_2pt] Error from save_rng_state, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );;
        EXIT(38);
      }

      /***************************************************************************
       * loop on up, dn flavor type
       ***************************************************************************/
      for ( int iflavor = 0; iflavor < 2; iflavor++ ) 
      {
        ...
        for( int i = 0; i < spin_color_dilution; i++) {
          ...
          if(exitstatus < 0) {
            fprintf(stderr, "[p2gg_vdisc_2pt] Error from invert, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
            EXIT(44);
          }
        ...
        for ( int isrc_mom = 0; isrc_mom < g_source_momentum_number; isrc_mom++ ) {
          ...
          if( source_momentum[0] == 0 &&
            ...
            /***************************************************************************
             * copy existing zero momentum propagator
             ***************************************************************************/
            if ( g_cart_id == 0 ) {
              fprintf( stdout, "# [p2gg_vdisc_2pt] copy existing zero mom propagator %s %d\n", __FILE__, __LINE__ );
            }
            for ( int i = 0; i < spin_color_dilution; i++ ) {
              memcpy ( stochastic_propagator_mom_list[i], stochastic_propagator_zero_list[i], sizeof_spinor_field );
            }
          ...
          for ( int isnk_gamma = 0; isnk_gamma < g_sink_gamma_id_number; isnk_gamma++ ) {
            ...
            if ( contr_x == NULL ) {
              fprintf(stderr, "[p2gg_vdisc_2pt] Error from init_1level_dtable %s %d\n", __FILE__, __LINE__);
              EXIT(3);
            }
            ...
            contract_twopoint_xdep ( contr_x, g_source_gamma_id_list[isrc_gamma], g_sink_gamma_id_list[isnk_gamma], 
                stochastic_propagator_zero_list, 
            ...
            if(exitstatus != 0) {
              fprintf(stderr, "[p2gg_vdisc_2pt] Error from momentum_projection, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
              EXIT(3);
            }
  
            sprintf ( data_tag, "/%c-gf-%c-gi/t%d/s%d/gf%d/gi%d/pix%dpiy%dpiz%d", flavor_tag[1-iflavor], flavor_tag[iflavor],
                gts, isample,
                g_sink_gamma_id_list[isnk_gamma], g_source_gamma_id_list[isrc_gamma],
...
#endif
            if(exitstatus != 0) {
              fprintf(stderr, "[p2gg_vdisc_2pt] Error from contract_write_to_file, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
              return(3);
            }
  ...
  if(io_proc == 2) {
    ...
    if( aff_status_str != NULL ) {
      fprintf(stderr, "[p2gg_vdisc_2pt] Error from aff_writer_close, status was %s %s %d\n", aff_status_str, __FILE__, __LINE__);
      EXIT(32);
    }
  }  /* end of if io_proc == 2 */
  ...
  if(g_cart_id==0) {
    g_the_time = time(NULL);
    fprintf(stdout, "# [p2gg_vdisc_2pt] %s# [p2gg_vdisc_2pt] end of run\n", ctime(&g_the_time));
    fprintf(stderr, "# [p2gg_vdisc_2pt] %s# [p2gg_vdisc_2pt] end of run\n", ctime(&g_the_time));
  }
