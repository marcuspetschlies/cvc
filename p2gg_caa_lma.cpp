...
#pragma omp parallel
{
  ...
  if ( exitstatus != 0 ) {
    fprinttf(stderr, "[] Error from init_clover, status was %d\n");
    EXIT(1);
  }
  ...
  for(isource_location=0; isource_location < g_source_location_number; isource_location++) {
    ...
    /* loop on up-type and dn-type flavor */
    ...
    for ( int mu = 0; mu < 5; mu++ ) {
      ...
      for( int is = 0; is < 12; is++ )
      {
        ...
        exitstatus = init_clover_eo_spincolor_pointsource_propagator (eo_spinor_field[120+is], eo_spinor_field[180+is],
            g_shifted_source_coords, i, gauge_field_with_phase, g_mzzinv_dn[0], (int)(shifted_source_proc_id == g_cart_id), eo_spinor_work[0]);
        if(exitstatus != 0 ) {
          fprintf(stderr, "[p2gg] Error from init_eo_spincolor_pointsource_propagator; status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
          EXIT(18);
        }
    ...
    exitstatus = Q_clover_eo_invert_subspace ( 
        &(eo_spinor_field[0]),   &(eo_sinor_field[60]),
        &(eo_spinor_field[0]),   &(eo_sinor_field[60]),
        60, eo_evecs_block, eo_evecs_lambdainv, evecs_num, gauge_field_with_phase, mzz,  mzzinv, 0, eo_spinor_work
    );
    if ( exitstatus != 0 ) {
     ...
     **********************************************************/
    exitstatus = Q_clover_eo_invert_subspace ( 
        &(eo_spinor_field[120]),   &(eo_sinor_field[180]),
        &(eo_spinor_field[120]),   &(eo_sinor_field[180]),
        60, eo_evecs_block, eo_evecs_lambdainv, evecs_num, gauge_field_with_phase, mzz,  mzzinv, 1, eo_spinor_work
    );
    if ( exitstatus != 0 ) {
      fprintf(stderr, "[p2gg] Error from Q_clover_eo_invert_subspace, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
      EXIT(28);
