...
int main(int argc, char **argv) {
  ...
  char const flavor_type_3pt[3][16] = { "DDDl-gc-ll-gi" , "DDDl-gc-ls-gi" ,  "DDDs-gc-sl-gi" };
  ...
  while ((c = getopt(argc, argv, "h?f:N:S:E:w:F:T:m:i:I:n:")) != -1) {
    ...
    case 'N':
      ...
      fprintf ( stdout, "# [avx3_conn_analyse] number of configs = %d\n", num_conf );
      break;
    case 'S':
      num_src_per_conf = atoi ( optarg );
      fprintf ( stdout, "# [avx3_conn_analyse] number of sources per config = %d\n", num_src_per_conf );
      break;
    case 'E':
      strcpy ( ensemble_name, optarg );
      fprintf ( stdout, "# [avx3_conn_analyse] ensemble_name set to %s\n", ensemble_name );
      break;
   case 'F':
      twop_fold_propagator = atoi ( optarg );
      fprintf ( stdout, "# [avx3_conn_analyse] twop fold_propagator set to %d\n", twop_fold_propagator );
      break;
    case 'w':
      write_data = atoi ( optarg );
      fprintf ( stdout, "# [avx3_conn_analyse] write_date set to %d\n", write_data );
      break;
    case 'T':
      sscanf( optarg, "%lf,%lf", twop_weight, twop_weight+1 );
      fprintf ( stdout, "# [avx3_conn_analyse] twop_weight set to %25.16e / %25.16e\n", twop_weight[0], twop_weight[1] );
      break;
/*    case 'B':
      sscanf( optarg, "%lf,%lf", fbwd_weight, fbwd_weight+1 );
      fprintf ( stdout, "# [avx3_conn_analyse] fbwd_weight set to %25.16e / %25.16e\n", fbwd_weight[0], fbwd_weight[1] );
      break;
    case 'M':
      sscanf( optarg, "%lf,%lf", mirror_weight, mirror_weight+1 );
      fprintf ( stdout, "# [avx3_conn_analyse] mirror_weight set to %25.16e / %25.16e\n", mirror_weight[0], mirror_weight[1] );
      break;
*/
    case 'm':
      g_mus = atof ( optarg );
    ...
    case 'i':
      ...
      fprintf ( stdout, "# [avx3_conn_analyse] flavor_id_2pt set to %d \n", flavor_id_2pt );
      break;
    case 'I':
      flavor_id_3pt = atoi ( optarg );
      fprintf ( stdout, "# [avx3_conn_analyse] flavor_id_3pt set to %d \n", flavor_id_3pt );
      break;
    case 'n':
      threep_operator_norm = atof ( optarg );
      fprintf ( stdout, "# [avx3_conn_analyse] threep_operator_norm set to %e \n", threep_operator_norm );
      break;
    case 'h':
    case '?':
    default:
   ...
   ******************************************************/
  if ( g_cart_id == 0 ) {
    fprintf(stdout, "# [avx3_conn_analyse] git version = %s\n", g_gitversion);
  }
   ...
   *********************************/
#ifdef HAVE_OPENMP
  if(g_cart_id == 0) fprintf(stdout, "# [avx3_conn_analyse] setting omp number of threads to %d\n", g_num_threads);
  omp_set_num_threads(g_num_threads);
#pragma omp parallel
{
  fprintf(stdout, "# [avx3_conn_analyse] proc%.4d thread%.4d using %d threads\n", g_cart_id, omp_get_thread_num(), omp_get_num_threads());
}
#else
  if(g_cart_id == 0) fprintf(stdout, "[avx3_conn_analyse] Warning, resetting global thread number to 1\n");
  g_num_threads = 1;
#endif

  if ( init_geometry() != 0 ) {
    fprintf(stderr, "[avx3_conn_analyse] Error from init_geometry %s %d\n", __FILE__, __LINE__);
    EXIT(4);
  }
  ...
  io_proc = get_io_proc ();
  if( io_proc < 0 ) {
    fprintf(stderr, "[avx3_conn_analyse] Error, io proc must be ge 0 %s %d\n", __FILE__, __LINE__);
    EXIT(14);
  }
  if ( g_verbose > 2 ) fprintf(stdout, "# [avx3_conn_analyse] proc%.4d has io proc id %d\n", g_cart_id, io_proc );
  ...
  if ( ofs == NULL ) {
    fprintf(stderr, "[avx3_conn_analyse] Error from fopen for filename %s %s %d\n", filename, __FILE__, __LINE__);
    EXIT(15);
  }

  int *** conf_src_list = init_3level_itable ( num_conf, num_src_per_conf, 6 );
  if ( conf_src_list == NULL ) {
    fprintf(stderr, "[avx3_conn_analyse] Error from init_3level_itable %s %d\n", __FILE__, __LINE__);
    EXIT(16);
  }
  char line[100];
  int count = 0;
  while ( fgets ( line, 100, ofs) != NULL && count < num_conf * num_src_per_conf ) {
    if ( line[0] == '#' ) {
      fprintf( stdout, "# [avx3_conn_analyse] comment %s\n", line );
      continue;
    }
    int itmp[5];
    char ctmp;
  ...
  switch ( flavor_id_2pt )
  {
    ...
    case 2:
      ...
      break;
    default:
      if ( io_proc == 2 ) fprintf ( stderr, "[avx3_conn_analyse] flavor_id_2pt = %d not implemented   %s %d\n", flavor_id_2pt, __FILE__, __LINE__ );
      EXIT(1);
      break;
  }
  ...
  switch ( flavor_id_3pt )
  {
    ...
    case 0:
      ...
      break;
    default:
      if ( io_proc == 2 ) fprintf ( stderr, "[avx3_conn_analyse] flavor_id_3pt = %d not implemented   %s %d\n", flavor_id_3pt, __FILE__, __LINE__ );
      EXIT(1);
      break;
  }
  ...
  if( twop == NULL ) {
    fprintf ( stderr, "[avx3_conn_analyse] Error from init_6level_dtable %s %d\n", __FILE__, __LINE__ );
    EXIT (24);
  }
    ...
    for( int isrc = 0; isrc < num_src_per_conf; isrc++ ) 
    {
      ...
      for ( int ipf = 0; ipf < g_sink_momentum_number; ipf++ ) {
        ...
        if ( twop_weight[0] != 0. ) 
        {
          ...
          if ( g_verbose > 3 ) fprintf ( stdout, "# [avx3_conn_analyse] key  = %s %s %d\n", key, __FILE__, __LINE__  );

          exitstatus = read_from_h5_file ( (void*)(buffer[0][0]), data_filename, key, "double", io_proc );
          if ( exitstatus != 0 ) {
            fprintf( stderr, "[avx3_conn_analyse] Error from read_from_h5_file for file %s key %s, status was %d %s %d\n", 
        ...
        if ( twop_weight[1] != 0. ) 
        {
          ...
          if ( g_verbose > 3 ) fprintf ( stdout, "# [avx3_conn_analyse] key2 = %s %s %d\n", key2, __FILE__, __LINE__  );

          exitstatus = read_from_h5_file ( (void*)(buffer[1][0]), data_filename, key2, "double", io_proc );
          if ( exitstatus != 0 ) {
            fprintf( stderr, "[avx3_conn_analyse] Error from read_from_h5_file for file %s key2 %s, status was %d %s %d\n",
...
#pragma omp parallel for
  ...
  show_time ( &ta, &tb, "avx3_conn_analyse", "read-twop-h5", g_cart_id == 0 );
  ...
  if( twop_orbit == NULL ) {
    fprintf ( stderr, "[avx3_conn_analyse] Error from init_4level_dtable %s %d\n", __FILE__, __LINE__ );
    EXIT (24);
  }
...
#pragma omp parallel for
    ...
    if ( num_conf < 6 ) 
    {
      fprintf ( stderr, "[avx3_conn_analyse] Error, too few observations for stats %s %d\n", __FILE__, __LINE__ );
      continue;
    } else {
      ...
      if ( exitstatus != 0 ) {
        fprintf ( stderr, "[avx3_conn_analyse] Error from apply_uwerr_real, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
        EXIT(1);
      }
      ...
      for ( int itau = 1; itau < Thp1/2; itau++ ) {
        ...
        if ( exitstatus != 0 ) {
          fprintf ( stderr, "[avx3_conn_analyse] Error from apply_uwerr_func, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
          EXIT(115);
        }
      }
  ...
  for ( int idt = 0; idt < g_sequential_source_timeslice_number; idt++ ) 
  {
     ...
     *
     * [D^fb3_lda D^fb2_nu D^fb1_mu q]^+ gamma_kappa q = q^+ D^fb1_mu D^fb2_nu D^fb3_lda gamma_kappa q
     *
     * idx_map[][0] = mu  ( first derivative applied )
     * idx_map[][1] = nu  ( second derivative applied )
     * idx_map[][2] = lda ( third derivative applied )
     *
     * forward / backward index for shift from 
     *
     * buffer[k][t][idd][fb3][fb2][fb1][kappa]
     **********************************************************/
    int const idx_num = 24;
    int const idx_map[24][4] = {
      { 0, 1, 2, 3 },  /*  0 */
      { 0, 1, 3, 2 },  /*  1 */
      { 0, 2, 1, 3 },  /*  2 */
      { 0, 2, 3, 1 },  /*  3 */
...
/**********************************************************/
    ...
    for ( int iconf = 0; iconf < num_conf; iconf++ ) 
    {
      double ****** buffer = init_6level_dtable ( 2, T_global, 24, 2, 2, 4 );
      ...
      for( int isrc = 0; isrc < num_src_per_conf; isrc++ ) 
      {
        ...
        if ( g_verbose > 2 ) fprintf ( stdout, "# [avx3_conn_analyse] reading from data filename %s %s %d\n", data_filename, __FILE__, __LINE__ );
        ...
        for ( int ipf = 0; ipf < g_sink_momentum_number; ipf++ ) {
          ...
          if ( twop_weight[0] != 0. ) 
          {
             ...
             * read h5 data key ...
             ***********************************************************/
            /* /DDDl-gc-ll-gi/mu0.0007/mu0.0007/mu-0.0007/dt16/px-1py-1pz-1 */
          ...
          for ( int iddd = 0; iddd < 24; iddd++ ) 
          {
            int const mu  = idx_map[iddd][0];
            int const nu  = idx_map[iddd][1];
            int const lda = idx_map[iddd][2];

            for ( int igc = 0; igc < 2; igc++ )
            {
              int const idx = iddd;
              int const kappa = idx_map[iddd][3];
              ...
              int const parity_sign = ( 2 * ( idx_map[idx][0] == 0 ) - 1 ) * ( 2 * ( idx_map[idx][1] == 0 ) - 1 ) * \
                                      ( 2 * ( idx_map[idx][2] == 0 ) - 1 ) * ( 2 * ( idx_map[idx][3] == 0 ) - 1 );
#pragma omp parallel for
              for ( int it = 0; it < T_global; it++ ) 
              {
                ...
                /* shift in lambda */
                int const itt_pl_lda = ( it + conf_src_list[iconf][isrc][2] + ( lda == 0 ) + 2*T_global ) % T_global;
                int const itt_mi_lda = ( it + conf_src_list[iconf][isrc][2] - ( lda == 0 ) + 2*T_global ) % T_global;
                ...
                /* shift in mu, lda */
                int const itt_pl_mu_pl_lda = ( it + conf_src_list[iconf][isrc][2] + ( mu == 0 ) + ( lda == 0 ) + 2*T_global ) % T_global;
                int const itt_pl_mu_mi_lda = ( it + conf_src_list[iconf][isrc][2] + ( mu == 0 ) - ( lda == 0 ) + 2*T_global ) % T_global;
                int const itt_mi_mu_pl_lda = ( it + conf_src_list[iconf][isrc][2] - ( mu == 0 ) + ( lda == 0 ) + 2*T_global ) % T_global;
                int const itt_mi_mu_mi_lda = ( it + conf_src_list[iconf][isrc][2] - ( mu == 0 ) - ( lda == 0 ) + 2*T_global ) % T_global;

                /* shift in nu, lda */
                int const itt_pl_nu_pl_lda = ( it + conf_src_list[iconf][isrc][2] + ( nu == 0 ) + ( lda == 0 ) + 2*T_global ) % T_global;
                int const itt_pl_nu_mi_lda = ( it + conf_src_list[iconf][isrc][2] + ( nu == 0 ) - ( lda == 0 ) + 2*T_global ) % T_global;
                int const itt_mi_nu_pl_lda = ( it + conf_src_list[iconf][isrc][2] - ( nu == 0 ) + ( lda == 0 ) + 2*T_global ) % T_global;
                int const itt_mi_nu_mi_lda = ( it + conf_src_list[iconf][isrc][2] - ( nu == 0 ) - ( lda == 0 ) + 2*T_global ) % T_global;

                /* shift in mu, nu, lda */
                int const itt_pl_mu_pl_nu_pl_lda = ( it + conf_src_list[iconf][isrc][2] + ( mu == 0 ) + ( nu == 0 ) + ( lda == 0 ) + 2*T_global ) % T_global;
                int const itt_pl_mu_pl_nu_mi_lda = ( it + conf_src_list[iconf][isrc][2] + ( mu == 0 ) + ( nu == 0 ) - ( lda == 0 ) + 2*T_global ) % T_global;
                int const itt_pl_mu_mi_nu_pl_lda = ( it + conf_src_list[iconf][isrc][2] + ( mu == 0 ) - ( nu == 0 ) + ( lda == 0 ) + 2*T_global ) % T_global;
                int const itt_pl_mu_mi_nu_mi_lda = ( it + conf_src_list[iconf][isrc][2] + ( mu == 0 ) - ( nu == 0 ) - ( lda == 0 ) + 2*T_global ) % T_global;
                int const itt_mi_mu_pl_nu_pl_lda = ( it + conf_src_list[iconf][isrc][2] - ( mu == 0 ) + ( nu == 0 ) + ( lda == 0 ) + 2*T_global ) % T_global;
                int const itt_mi_mu_pl_nu_mi_lda = ( it + conf_src_list[iconf][isrc][2] - ( mu == 0 ) + ( nu == 0 ) - ( lda == 0 ) + 2*T_global ) % T_global;
                int const itt_mi_mu_mi_nu_pl_lda = ( it + conf_src_list[iconf][isrc][2] - ( mu == 0 ) - ( nu == 0 ) + ( lda == 0 ) + 2*T_global ) % T_global;
                int const itt_mi_mu_mi_nu_mi_lda = ( it + conf_src_list[iconf][isrc][2] - ( mu == 0 ) - ( nu == 0 ) - ( lda == 0 ) + 2*T_global ) % T_global;
                ...
                for ( int k = 0; k <= 1; k++ )              /* parity components */
                {
                  ...
                  {
                      /*               from [ (fwd+bwd)/2 ]^3   from [ (Dright-Dleft)/2 ]^3 */
                      dtmp[k][ireim] = 0.125                  * 0.125 * ( 
                        /*
                         * (1)
                         *   --> --> -->
                         * +  D   D   D
                         *
                         */
                       + buffer[k][itt_pl_mu_pl_nu_pl_lda][iddd][1][1][2+ireim]
                       - buffer[k][itt_pl_mu_pl_nu_mi_lda][iddd][0][1][2+ireim]
                       - buffer[k][itt_pl_mu_mi_nu_pl_lda][iddd][1][0][2+ireim]
                       + buffer[k][itt_pl_mu_mi_nu_mi_lda][iddd][0][0][2+ireim]
                       - buffer[k][itt_mi_mu_pl_nu_pl_lda][iddd][1][1][0+ireim]
                       + buffer[k][itt_mi_mu_pl_nu_mi_lda][iddd][0][1][0+ireim]
                       + buffer[k][itt_mi_mu_mi_nu_pl_lda][iddd][1][0][0+ireim]
                       - buffer[k][itt_mi_mu_mi_nu_mi_lda][iddd][0][0][0+ireim]
                        /*
                         * (2)
                         *   <-- <-- <--
                         * -  D   D   D
                       ...
                       + buffer[k][itt][iddd][1][0][2+ireim]
                       - buffer[k][itt][iddd][0][1][0+ireim]
                       ...
                       + buffer[k][itt_pl_nu_mi_lda][iddd][0][1][2+ireim]
                       - buffer[k][itt_mi_nu_pl_lda][iddd][1][0][0+ireim]
                       ...
                       - buffer[k][itt_pl_lda][iddd][1][0][2+ireim]
                       - buffer[k][itt_mi_lda][iddd][0][0][0+ireim]
                       + buffer[k][itt_mi_lda][iddd][0][0][2+ireim]
                       - buffer[k][itt_pl_lda][iddd][1][1][0+ireim]
                       + buffer[k][itt_pl_lda][iddd][1][1][2+ireim]
                       + buffer[k][itt_mi_lda][iddd][0][1][0+ireim]
                       - buffer[k][itt_mi_lda][iddd][0][1][2+ireim]
                        /*
                         * (5)
                         *   --> <-- -->
                         * -  D   D   D
                       ...
                       + buffer[k][itt_pl_mu_mi_lda][iddd][0][1][2+ireim]
                       - buffer[k][itt_mi_mu_pl_lda][iddd][1][0][0+ireim]
                       ...
                       + buffer[k][itt_pl_mu_mi_nu][iddd][1][0][2+ireim]
                       - buffer[k][itt_mi_mu_pl_nu][iddd][0][1][0+ireim]
                       ...
                       - buffer[k][itt_pl_mu][iddd][1][0][2+ireim]
                       - buffer[k][itt_pl_mu][iddd][0][1][2+ireim]
                       + buffer[k][itt_pl_mu][iddd][1][1][2+ireim]
                       - buffer[k][itt_mi_mu][iddd][0][0][0+ireim]
                       + buffer[k][itt_mi_mu][iddd][1][0][0+ireim]
                       + buffer[k][itt_mi_mu][iddd][0][1][0+ireim]
                       - buffer[k][itt_mi_mu][iddd][1][1][0+ireim]
                        /*
                         * (8)
                         *   <-- --> <--
                         * +  D   D   D
                       ...
                       - buffer[k][itt_pl_nu][iddd][1][1][0+ireim]
                       - buffer[k][itt_mi_nu][iddd][0][0][0+ireim]
                       + buffer[k][itt_mi_nu][iddd][1][0][0+ireim]
                       - buffer[k][itt_pl_nu][iddd][0][1][2+ireim]
                       + buffer[k][itt_pl_nu][iddd][1][1][2+ireim]
                       + buffer[k][itt_mi_nu][iddd][0][0][2+ireim]
                       - buffer[k][itt_mi_nu][iddd][1][0][2+ireim]
                    );
    ...
    gettimeofday ( &tb, (struct timezone *)NULL );
    show_time ( &ta, &tb, "avx3_conn_analyse", "read-threep-h5", g_cart_id == 0 );
    ...
    for ( int imom = 0; imom < g_sink_momentum_number; imom++ )
    {
      for ( int k = 0; k < 24; k++ )
      {
        for ( int ireim = 0; ireim < 2; ireim++ )
        {
          ...
          sprintf ( obs_name, "threep.%s.conn.gddd%d%d%d%d.dtsnk%d.PX%d_PY%d_PZ%d.%s",
                    flavor_type_3pt[flavor_id_3pt],
                    idx_map[k][0], idx_map[k][1], idx_map[k][2], idx_map[k][3],
                    g_sequential_source_timeslice_list[idt],
...
#pragma omp parallel for
    ...
    double ***** threep_sym = init_5level_dtable ( g_sink_momentum_number, num_conf, num_src_per_conf, T_global, 2 );
    if ( threep_sym == NULL ) {
      fprintf( stderr, "[avx3_conn_analyse] Error from init_Xlevel_dtable %s %d\n", __FILE__, __LINE__ );
      EXIT(1);
    }
        ...
        for ( int isrc = 0; isrc < num_src_per_conf; isrc++ )
        {
          for ( int it = 0; it < T_global; it++ )
          {
            for ( int ireim = 0; ireim < 2; ireim++ )
            {
              ...
              threep_sym[imom][iconf][isrc][it][ireim] = dtmp / 24.;
            }
          }
        }
      }
          ...
          for ( int ireim = 0; ireim < 2; ireim++ )
          {
            ...
            for ( int iconf = 0; iconf < num_conf; iconf++ )
            {
              for ( int isrc = 0; isrc < num_src_per_conf; isrc++ )
              {
                for ( int it = 0; it < T_global; it++ )
                {
                  fprintf (fs, "%3d  %25.16e   %c %6d   %3d %3d %3d %3d\n", it, threep_sym[imom][iconf][isrc][it][ireim],
                     conf_src_list[iconf][isrc][0],
                     conf_src_list[iconf][isrc][1],
                     conf_src_list[iconf][isrc][2], conf_src_list[iconf][isrc][3], conf_src_list[iconf][isrc][4], conf_src_list[iconf][isrc][5] );
                }
          ...
          for ( int it = 0; it < T_global; it++ )
          {
            for ( int ireim = 0; ireim < 2; ireim++ )
            {
              ...
              for ( int imom = 0; imom < g_sink_momentum_number; imom++ )
              {
                ...
                if ( ip[0] * ip[1] * ip[2] == 0 )
                {
                  if ( g_verbose > 2 ) fprintf ( stdout, "# [avx3_conn_analyse] skip p = %3d %3d %3d \n", ip[0], ip[1], ip[2] );
                  continue;
                } else 
                {
                  counter++;
                  ...
                  double const p[3] = {
                    ip[0] * 2. * M_PI / (double)LX_global,
                    ip[1] * 2. * M_PI / (double)LY_global,
                    ip[2] * 2. * M_PI / (double)LZ_global };

                  double const norm = p[0] * p[1] * p[2];

                  dtmp += threep_sym[imom][iconf][isrc][it][ireim] / norm;
                }
            
              }  /* end of loop on momenta */

              /* normalize by number of included 3-momentum vectors  */
              threep_orbit[iconf][isrc][it][ireim] = dtmp / (double)counter;
        ...
        if ( write_data == 1) 
        {
          ...
          for ( int iconf = 0; iconf < num_conf; iconf++ )
          {
            for ( int isrc = 0; isrc < num_src_per_conf; isrc++ )
            {
              for ( int it = 0; it < T_global; it++ )
              {
                fprintf (fs, "%3d  %25.16e   %c %6d   %3d %3d %3d %3d\n", it, threep_orbit[iconf][isrc][it][ireim],
                   conf_src_list[iconf][isrc][0],
                   conf_src_list[iconf][isrc][1],
                   conf_src_list[iconf][isrc][2], conf_src_list[iconf][isrc][3], conf_src_list[iconf][isrc][4], conf_src_list[iconf][isrc][5] );
              }
...
#pragma omp parallel for
        for ( int iconf = 0; iconf < num_conf; iconf++ ) {
          ...
          for ( int it = 0; it < T_global; it++ ) {
            double dtmp = 0.;
            for ( int isrc = 0; isrc < num_src_per_conf; isrc++ ) {
              dtmp += threep_orbit[iconf][isrc][it][ireim];
            }
            data[iconf][it] = dtmp / (double)num_src_per_conf;
          }
        }
  ...
  show_time ( &starttime, &endtime, "avx3_conn_analyse", "avx3_conn_analyse", g_cart_id == 0 );
