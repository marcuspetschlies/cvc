...
*/
  if(format != 2) {
    ...
    sprintf(filename, "%s.%.4d.%.4d", filename_prefix, iconf, g_source_location);
  } else {
...
if(mode==0 || mode==1) {
  ...
  sprintf(filename, "pi%1d.%.2d.%.4d.%.4d", proj_type, 1, Nconf,g_source_location);
  if( (ofs=fopen(filename, "w")) == (FILE*)NULL ) {
    fprintf(stderr, "Error on opening file %s\n", filename);
    return(110);
  }
