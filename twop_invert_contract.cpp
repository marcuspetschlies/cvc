...
int main(int argc, char **argv) {
  ...
  /* fprintf(stdout, "# [twop_invert_contract] Reading input from file %s\n", filename); */
  read_input_parser(filename);

#ifdef HAVE_TMLQCD_LIBWRAPPER

  fprintf(stdout, "# [twop_invert_contract] calling tmLQCD wrapper init functions\n");
   ...
   ***************************************************************************/
  if ( g_cart_id == 0 ) {
    fprintf(stdout, "# [twop_invert_contract] git version = %s\n", g_gitversion);
  }
   ...
   ***************************************************************************/
  if ( init_geometry() != 0 ) {
    fprintf(stderr, "[twop_invert_contract] Error from init_geometry %s %d\n", __FILE__, __LINE__);
    EXIT(4);
  }
  ...
  if(!(strcmp(gaugefilename_prefix,"identity")==0)) {
    ...
    if(g_cart_id==0) fprintf(stdout, "# [twop_invert_contract] reading gauge field from file %s\n", filename);
    exitstatus = read_lime_gauge_field_doubleprec(filename);
  } else {
    /* initialize unit matrices */
    if(g_cart_id==0) fprintf(stdout, "\n# [twop_invert_contract] initializing unit matrices\n");
    exitstatus = unit_gauge_field ( g_gauge_field, VOLUME );
  }
  if(exitstatus != 0) {
    fprintf ( stderr, "[twop_invert_contract] Error initializing gauge field, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
    EXIT(8);
  }
#else
  Nconf = g_tmLQCD_lat.nstore;
  if(g_cart_id== 0) fprintf(stdout, "[twop_invert_contract] Nconf = %d\n", Nconf);
  ...
  if( g_gauge_field == NULL) {
    fprintf(stderr, "[twop_invert_contract] Error, g_gauge_field is NULL %s %d\n", __FILE__, __LINE__);
    EXIT(7);
  }
#endif
  ...
  if(exitstatus != 0) {
    fprintf(stderr, "[twop_invert_contract] Error from gauge_field_eq_gauge_field_ti_phase, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(38);
  }
  ...
  if(exitstatus != 0) {
    fprintf(stderr, "[twop_invert_contract] Error from plaquetteria, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(38);
  }
  ...
  if ( exitstatus != 0 ) {
    fprintf(stderr, "[twop_invert_contract] Error from init_clover, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
    EXIT(1);
  }
  ...
  if( N_Jacobi > 0 ) {
    ...
    if ( N_ape > 0 ) {
      ...
      if(exitstatus != 0) {
        fprintf(stderr, "[twop_invert_contract] Error from APE_Smearing, status was %d\n", exitstatus);
        EXIT(47);
      }
    }  /* end of if N_aoe > 0 */
  ...
  io_proc = get_io_proc ();
  if( io_proc < 0 ) {
    fprintf(stderr, "[twop_invert_contract] Error, io proc must be ge 0 %s %d\n", __FILE__, __LINE__);
    EXIT(14);
  }
  ...
  if ( spinor_work == NULL ) {
    fprintf(stderr, "[twop_invert_contract] Error from init_2level_dtable %s %d\n", __FILE__, __LINE__ );
    EXIT(1);
  }
  ...
  if ( exitstatus != 0 ) {
    fprintf(stderr, "[twop_invert_contract] Error from init_rng_state %s %d\n", __FILE__, __LINE__ );;
    EXIT( 50 );
  }
  ...
  for( int isource_location = 0; isource_location < g_source_location_number; isource_location++ ) {
    ...
    if( exitstatus != 0 ) {
      fprintf(stderr, "[twop_invert_contract] Error from get_timeslice_source_info status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
      EXIT(123);
    }
    ...
    if(io_proc == 2) {
      ...
      if( aff_status_str != NULL ) {
        fprintf(stderr, "[twop_invert_contract] Error from aff_writer, status was %s %s %d\n", aff_status_str, __FILE__, __LINE__);
        EXIT(15);
      }
    }  /* end of if io_proc == 2 */
    ...
    if ( ! g_read_source ) {
      ...
      if(exitstatus != 0) {
        fprintf(stderr, "[twop_invert_contract] Error from init_rng_stat_file status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
        EXIT(38);
      }
    }
    */
    ...
    for ( int isample = 0; isample < g_nsample_oet; isample++ ) {
      ...
      if(exitstatus != 0) {
        fprintf(stderr, "[twop_invert_contract] Error from sync_rng_state, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
        EXIT(38);
      }
      ...
      if ( g_read_source ) {
        for ( int i = 0; i < spin_color_dilution; i++ ) {
          ...
          if ( ( exitstatus = read_lime_spinor( stochastic_source_list[i], filename, 0) ) != 0 ) {
            fprintf(stderr, "[twop_invert_contract] Error from read_lime_spinor, status was %d\n", exitstatus);
            EXIT(2);
          }
        }
        ...
        if( exitstatus != 0 ) {
          fprintf(stderr, "[twop_invert_contract] Error from init_timeslice_source_oet, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
          EXIT(64);
        }
        ...
        if ( g_write_source ) {
          for ( int i = 0; i < spin_color_dilution; i++ ) {
            ...
            if ( ( exitstatus = write_propagator( stochastic_source_list[i], filename, 0, g_propagator_precision) ) != 0 ) {
              fprintf(stderr, "[twop_invert_contract] Error from write_propagator, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
              EXIT(2);
            }
          }
        }
      ...
      if(exitstatus != 0) {
        fprintf(stderr, "[twop_invert_contract] Error from get_rng_state, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
        EXIT(38);
      }

      exitstatus = save_rng_state ( 0, NULL );
      if ( exitstatus != 0 ) {
        fprintf(stderr, "[twop_invert_contract] Error from save_rng_state, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );;
        EXIT(38);
      }
      ...
      for( int i = 0; i < spin_color_dilution; i++) {
        ...
        if ( N_Jacobi > 0 ) {
          ...
          if(exitstatus != 0) {
            fprintf(stderr, "[twop_invert_contract] Error from Jacobi_Smearing, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
            return(11);
          }
        }
      ...
      for ( int isrc_mom = 0; isrc_mom < g_sink_momentum_number; isrc_mom++ ) {
        ...
        for ( int isnk_gamma = 0; isnk_gamma < g_sink_gamma_id_number; isnk_gamma++ ) {
          ...
          if ( contr_x == NULL ) {
            fprintf(stderr, "[twop_invert_contract] Error from init_1level_dtable %s %d\n", __FILE__, __LINE__);
            EXIT(3);
          }
    ...
    if(io_proc == 2) {
      ...
      if( aff_status_str != NULL ) {
        fprintf(stderr, "[twop_invert_contract] Error from aff_writer_close, status was %s %s %d\n", aff_status_str, __FILE__, __LINE__);
        EXIT(32);
      }
    }  /* end of if io_proc == 2 */
  ...
  if(g_cart_id==0) {
    g_the_time = time(NULL);
    fprintf(stdout, "# [twop_invert_contract] %s# [twop_invert_contract] end of run\n", ctime(&g_the_time));
    fprintf(stderr, "# [twop_invert_contract] %s# [twop_invert_contract] end of run\n", ctime(&g_the_time));
  }
