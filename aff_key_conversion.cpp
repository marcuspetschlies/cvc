...
void aff_key_conversion (char*key, char * const tag, int const i_sample, int const pi2[3], int const pf1[3], int const pf2[3], int const source_coords[4], int const gamma_id, int const C_gamma_id,  int const i_spin ) {
  ...
  if ( strcmp( tag, "w_1_xi") == 0 ) {
    ...
    const char * stoch_field = use_oet ? "g5.phi" : "xi";

    sprintf( key, "/v3/t%.2dx%.2dy%.2dz%.2d/%s-g%.2d-u/sample%.2d/px%.2dpy%.2dpz%.2d",
        source_coords[0], source_coords[1], source_coords[2], source_coords[3], stoch_field,
        C_gid, i_sample,
        pf2[0], pf2[1], pf2[2]);

  } else if ( strcmp( tag, "w_1_phi") == 0 ) {
    /* /w_1_phi/sample02/pi2x00pi2y00pi2z01/pf1x00pf1y00pf1z01/t09x06y07z00/g06 */
    const char * stoch_field = use_oet ? "g5.xi" : "phi";

    sprintf(key, "/v2/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/%s-g%.2d-u-ud/sample%.2d/px%.2dpy%.2dpz%.2d",
        source_coords[0], source_coords[1], source_coords[2], source_coords[3],
        pi2[0], pi2[1], pi2[2], stoch_field, C_gid, i_sample,
        pf1[0], pf1[1], pf1[2]);
    ...
    sprintf(key, "/v2/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/%s-g%.2d-ud-u/sample%.2d/px%.2dpy%.2dpz%.2d",
        source_coords[0], source_coords[1], source_coords[2], source_coords[3],
        pi2[0], pi2[1], pi2[2], stoch_field, C_gid, i_sample,
        pf1[0], pf1[1], pf1[2]);
    ...
    sprintf(key, "/v3/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/%s-g%.2d-ud/sample%.2d/px%.2dpy%.2dpz%.2d",
        source_coords[0], source_coords[1], source_coords[2], source_coords[3],
        pi2[0], pi2[1], pi2[2], stoch_field, C_gid, i_sample,
        pf2[0], pf2[1], pf2[2] );
    ...
    sprintf(key, "/v2/t%.2dx%.2dy%.2dz%.2d/%s-g%.2d-u-u/sample%.2d/px%.2dpy%.2dpz%.2d",
        source_coords[0], source_coords[1], source_coords[2], source_coords[3], stoch_field,
        C_gid, i_sample,
        pf1[0], pf1[1], pf1[2]);
    ...
    const char * stoch_field = "phi";

    sprintf(key, "/v2-oet/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/%s-g%.2d-d-u/sample%.2d/d%d/px%.2dpy%.2dpz%.2d",
        source_coords[0], source_coords[1], source_coords[2], source_coords[3],
        pi2[0], pi2[1], pi2[2], stoch_field, C_gid, i_sample, i_spin,
        pf1[0], pf1[1], pf1[2]);
    ...
    sprintf(key, "/v4-oet/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/%s-g%.2d-d-u/sample%.2d/d%d/px%.2dpy%.2dpz%.2d", 
        source_coords[0], source_coords[1], source_coords[2], source_coords[3],
        pi2[0], pi2[1], pi2[2], stoch_field, C_gid, i_sample, i_spin, pf1[0], pf1[1], pf1[2]);

  } else if ( strcmp( tag, "z_1_xi") == 0 ) {
    /* /z_1_xi/sample00/pf2x00pf2y00pf2z-01/t09x06y07z00 */
    const char * stoch_field = "g5.phi";

    sprintf(key, "/v3-oet/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/%s-g%.2d-u/sample%.2d/d%d/px%.2dpy%.2dpz%.2d",
        source_coords[0], source_coords[1], source_coords[2], source_coords[3],
        pi2[0], pi2[1], pi2[2], stoch_field, C_gid, i_sample, i_spin,
        pf2[0], pf2[1], pf2[2] );
  }
