...
int main(int argc, char **argv) {
  ...
  while ((c = getopt(argc, argv, "h?vf:n:c:")) != -1) {
    ...
    case 'c':
      ...
      fprintf(stdout, "# [test_gsp_loop] use first %d eigenvectors for contraction\n", evecs_num_contract);
      break;
    case 'h':
    case '?':
    default:
...
#pragma omp parallel private(nthreads,threadid)
{
...
#else
    sprintf(filename, "%s.%.4d.%.4d", "gsp_loop", Nconf, evecs_num_contract);
  ...
  for(i2pt=0; i2pt < g_m_m_2pt_num; i2pt++) {
    ...
    if(g_cart_id == 0) fprintf(stdout, "# [test_gsp_loop] %d to (%d, %d)\n", g_m_m_2pt_list[i2pt].gi, i_gi, i_si);
    ...
    /* TEST */
    /* gsp_printf (gsp_v_w[0][0], evecs_num, "gsp_v_w", stdout); */
    ...
    /* TEST */
    /* gsp_printf (gsp_xv_xw[0][0], evecs_num, "gsp_xv_xw", stdout); */
    ...
    for(i_ti = 0; i_ti<T; i_ti++) {
      ...
      co_eq_tr_gsp (&w, gsp_xv_xw[0][0][i_ti], evecs_eval, evecs_num_contract);
      correlator[2*x0  ] = w.re * i_si;
      correlator[2*x0+1] = w.im * i_si;

      co_eq_tr_gsp (&w, gsp_v_w[0][0][i_ti],   evecs_eval, evecs_num_contract);
      correlator2[2*x0  ] = w.re * i_si;
      correlator2[2*x0+1] = w.im * i_si;
      
    }  /* end of loop on tf */
    retime = _GET_TIME;
    if(g_cart_id == 0) fprintf(stdout, "# [test_gsp_loop] time for loop reduction = %e seconds\n", retime - ratime);
    ...
    if(io_proc == 2) {
       ...
       ***********************************************/
      items = T_global;
      sprintf(aff_buffer_path, "/xv-xw/g%.2d/px%.2dpy%.2dpz%.2d", 
      ...
      if(status != 0) {
        fprintf(stderr, "[test_gsp_loop] Error from aff_node_put_complex, status was %d\n", status);
        EXIT(26);
      }
      sprintf(aff_buffer_path, "/v_w/g%.2d/px%.2dpy%.2dpz%.2d", 
          g_m_m_2pt_list[i2pt].gi, g_m_m_2pt_list[i2pt].pi[0], g_m_m_2pt_list[i2pt].pi[1], g_m_m_2pt_list[i2pt].pi[2]);
      /* if(g_cart_id == 0) fprintf(stdout, "# [test_gsp_loop] current aff path = %s\n", aff_buffer_path); */
      affdir = aff_writer_mkpath(affw, affn, aff_buffer_path);
