...
void make_key_string ( char * key, twopoint_function_type *tp, const char * type, const char * diagram_name , const int * sx ) {
  ...
  if ( strcmp ( tp_type, "b-b" ) == 0 ) {
    ...
    sprintf ( key, "/%s/%s/gf2%.2d_%.2d/pf2x%.2dpf2y%.2dpf2z%.2d/gf1%.2d_%.2d/pf1x%.2dpf1y%.2dpf1z%.2d/gi2%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/gi1%.2d_%.2d/pi1x%.2dpi1y%.2dpi1z%.2d",
        tp->name, tp->fbwd,
...
void make_correlator_string ( char * name , twopoint_function_type * tp , const char * type ) {
  ...
  if ( strcmp ( tp_type, "b-b" ) == 0 ) {
    ...
    sprintf ( key, "%s.%s.gf1_%s_%s.pf1x%.2dpf1y%.2dpf1z%.2d.gi1_%s_%s.pi1x%.2dpi1y%.2dpi1z%.2d", tp->name, tp->fbwd,
