...
int main(int argc, char **argv) {
  ...
  const char outfile_prefix[] = "njjn_w_pv";
  ...
  char const sequential_gamma_tag[2][3] = { "va", "av" };
  ...
  */
  ...
  fprintf(stdout, "# [njjn_w_pv_invert_contract] calling tmLQCD wrapper init functions\n");
   ...
   ***************************************************************************/
  if ( g_cart_id == 0 ) {
    fprintf(stdout, "# [njjn_w_pv_invert_contract] git version = %s\n", g_gitversion);
  }
   ...
   ***************************************************************************/
#ifdef HAVE_OPENMP
  if(g_cart_id == 0) fprintf(stdout, "# [njjn_w_pv_invert_contract] setting omp number of threads to %d\n", g_num_threads);
  omp_set_num_threads(g_num_threads);
#pragma omp parallel
{
  fprintf(stdout, "# [njjn_w_pv_invert_contract] proc%.4d thread%.4d using %d threads\n", g_cart_id, omp_get_thread_num(), omp_get_num_threads());
}
#else
  if(g_cart_id == 0) fprintf(stdout, "[njjn_w_pv_invert_contract] Warning, resetting global thread number to 1\n");
  g_num_threads = 1;
#endif

  if ( init_geometry() != 0 ) {
    fprintf(stderr, "[njjn_w_pv_invert_contract] Error from init_geometry %s %d\n", __FILE__, __LINE__);
    EXIT(4);
  }
  ...
  if(!(strcmp(gaugefilename_prefix,"identity")==0)) {
    ...
    if(g_cart_id==0) fprintf(stdout, "# [njjn_w_pv_invert_contract] reading gauge field from file %s\n", filename);
    exitstatus = read_lime_gauge_field_doubleprec(filename);
  } else {
    /* initialize unit matrices */
    if(g_cart_id==0) fprintf(stdout, "\n# [njjn_w_pv_invert_contract] initializing unit matrices\n");
    exitstatus = unit_gauge_field ( g_gauge_field, VOLUME );
  }
#else
  Nconf = g_tmLQCD_lat.nstore;
  if(g_cart_id== 0) fprintf(stdout, "[njjn_w_pv_invert_contract] Nconf = %d\n", Nconf);
  ...
  if( g_gauge_field == NULL) {
    fprintf(stderr, "[njjn_w_pv_invert_contract] Error, g_gauge_field is NULL %s %d\n", __FILE__, __LINE__);
    EXIT(7);
  }
#endif
  ...
  if(exitstatus != 0) {
    fprintf(stderr, "[njjn_w_pv_invert_contract] Error from gauge_field_eq_gauge_field_ti_phase, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(38);
  }
  ...
  if ( exitstatus != 0 ) {
    fprintf(stderr, "[njjn_w_pv_invert_contract] Error from init_clover, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
    EXIT(1);
  }
  ...
  io_proc = get_io_proc ();
  if( io_proc < 0 ) {
    fprintf(stderr, "[njjn_w_pv_invert_contract] Error, io proc must be ge 0 %s %d\n", __FILE__, __LINE__);
    EXIT(14);
  }
  fprintf(stdout, "# [njjn_w_pv_invert_contract] proc%.4d has io proc id %d\n", g_cart_id, io_proc );
  ...
  if ( exitstatus != 0 ) {
    fprintf(stderr, "[njjn_w_pv_invert_contract] Error from init_rng_stat_file %s %d\n", __FILE__, __LINE__ );;
    EXIT( 50 );
  }
  ...
  if ( g_coherent_source_number * g_nsample_oet > 0 ) {
    ...
    if( scalar_field == NULL ) {
      fprintf(stderr, "[njjn_w_pv_invert_contract] Error from init_Xlevel_dtable %s %d\n", __FILE__, __LINE__);
      EXIT(132);
    }
  }
  ...
  if ( ! read_scalar_field  && ( g_coherent_source_number * g_nsample_oet > 0 ) ) {
    ...
    if ( write_scalar_field ) {
      ...
      for ( int i = 0; i < g_coherent_source_number * g_nsample_oet; i++ ) {
        ...
        if ( exitstatus != 0  ) {
          fprintf ( stderr, "[njjn_w_pv_invert_contract] Error write_lime_contraction, status was %d  %s %d\n", exitstatus, __FILE__, __LINE__ );
          EXIT(12);
        }
      }
    ...
    for ( int i = 0; i < g_coherent_source_number * g_nsample_oet; i++ ) {
      ...
      if ( exitstatus != 0  ) {
        fprintf ( stderr, "[njjn_w_pv_invert_contract] Error read_lime_contraction, status was %d  %s %d\n", exitstatus, __FILE__, __LINE__ );
        EXIT(12);
      }
    }
...
#  endif
  {
    fprintf(stderr, "[njjn_w_pv_invert_contract] Error from invert, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(12);
  }
  ...
  if( N_Jacobi > 0 ) {
    ...
    if ( N_ape > 0 ) {
      ...
      if(exitstatus != 0) {
        fprintf(stderr, "[njjn_w_pv_invert_contract] Error from APE_Smearing, status was %d\n", exitstatus);
        EXIT(47);
      }
#ifndef _SMEAR_QUDA 
    }  /* end of if N_ape > 0 */
  ...
  for( int isource_location = 0; isource_location < g_source_location_number; isource_location++ ) {
    ...
    if( exitstatus != 0 ) {
      fprintf(stderr, "[njjn_w_pv_invert_contract] Error from get_point_source_info status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
      EXIT(123);
    }
     ...
     ***************************************************************************/
      ...
      fprintf(stdout, "# [njjn_w_pv_invert_contract] writing data to file %s\n", filename);
      affw = aff_writer(filename);
      const char * aff_status_str = aff_writer_errstr ( affw );
      if( aff_status_str != NULL ) {
        fprintf(stderr, "[njjn_w_pv_invert_contract] Error from aff_writer, status was %s %s %d\n", aff_status_str, __FILE__, __LINE__);
        EXIT(15);
      }
#else
      fprintf(stderr, "[njjn_w_pv_invert_contract] Error, no outupt variant selected %s %d\n",  __FILE__, __LINE__);
      EXIT(15);
#endif
    }  /* end of if io_proc == 2 */
    ...
    for ( int icoh = 0; icoh < g_coherent_source_number; icoh++ ) {
      ...
      for ( int iflavor = 0; iflavor < 2; iflavor++ ) 
      {
        ...
        if(exitstatus != 0) {
          fprintf(stderr, "[njjn_w_pv_invert_contract] Error from point_source_propagator, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
          EXIT(12);
        }
      
        gettimeofday ( &tb, (struct timezone *)NULL );
        show_time ( &ta, &tb, "njjn_w_pv_invert_contract", "forward-light-smear-invert-check", g_cart_id == 0 );
        ...
        for ( int i = 0; i < 12; i++ ) {
          ...
          if(exitstatus != 0) {
            fprintf(stderr, "[njjn_w_pv_invert_contract] Error from Jacobi_Smearing, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
            return(11);
          }
        }
      
        gettimeofday ( &tb, (struct timezone *)NULL );
        show_time ( &ta, &tb, "njjn_w_pv_invert_contract", "forward-light-sink-smear", g_cart_id == 0 );
        ...
        if ( g_write_propagator ) {
          ...
          for ( int i = 0; i < 12; i++ ) {
            ...
            if ( ( exitstatus = write_propagator( propagator[icoh][iflavor][i], filename, 0, g_propagator_precision) ) != 0 ) {
              fprintf(stderr, "[njjn_w_pv_invert_contract] Error from write_propagator, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
              EXIT(2);
            }
          }
        }
      ...
      for ( int iflavor = 0; iflavor < 2; iflavor++ ) {
        ...
        if ( vx == NULL ) {
          fprintf(stderr, "[njjn_w_pv_invert_contract] Error from init_2level_dtable, %s %d\n", __FILE__, __LINE__);
          EXIT(47);
        }
        ...
        if ( vp == NULL ) {
          fprintf(stderr, "[njjn_w_pv_invert_contract] Error from init_3level_dtable %s %d\n", __FILE__, __LINE__ );
          EXIT(47);
        }
        ...
        for ( int if2 = 0; if2 < gamma_f1_number; if2++ ) {
          ...
          if ( exitstatus != 0 ) {
            fprintf(stderr, "[njjn_w_pv_invert_contract] Error from reduce_project_write, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
            EXIT(48);
          }
        ...
        gettimeofday ( &tb, (struct timezone *)NULL );
        show_time ( &ta, &tb, "njjn_w_pv_invert_contract", "n1-n2-reduce-project-write", g_cart_id == 0 );
    ...
    if ( g_cart_id == 0 && g_verbose > 2 ) fprintf(stdout, "# [njjn_w_pv_invert_contract] start seq source mom %3d %3d %3d  %s %d\n", 
        ...
        for ( int ig = 0; ig < 4; ig++ )
        {
          ...
          for ( int iflavor = 0; iflavor < 2; iflavor++ ) 
          {
            ...
            for ( int icol2 = 0; icol2 < 3; icol2++ ) 
            {
              ...
              for ( int iva = 0; iva < 2; iva++ )
              {
                ...
                for ( int icoh = 0; icoh < g_coherent_source_number; icoh++)
                {
                    ...
                    exitstatus = prepare_sequential_fht_twinpeak_source_crossed ( sequential_source_accum, propagator[icoh][iflavor], scalar_field[icoh][isample],
                        sequential_gamma_id[iva][ig], icol1, icol2 );
                    if ( exitstatus != 0 ) {
                      fprintf ( stderr, "[njjn_w_pv_invert_contract] Error from prepare_sequential_fht_twinpeak_source_crossed, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
                      EXIT(123);
                    }
                  ...
                  gettimeofday ( &tb, (struct timezone *)NULL );
                  show_time ( &ta, &tb, "njjn_w_pv_invert_contract", "prepare-sequential-fht-w-source", g_cart_id == 0 );
                  ...
                  gettimeofday ( &ta, (struct timezone *)NULL );
                  exitstatus = prepare_propagator_from_source ( sequential_propagator[iflavor][iva][icol1][icol2], sequential_source, 12, iflavor, 0, 1, gauge_field_smeared,
                      check_propagator_residual, gauge_field_with_phase, lmzz, NULL );
                  if ( exitstatus != 0 ) {
                    fprintf ( stderr, "[njjn_w_pv_invert_contract] Error from prepare_propagator_from_source, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
                    EXIT(123);
                  }

                  gettimeofday ( &tb, (struct timezone *)NULL );
                  show_time ( &ta, &tb, "njjn_w_pv_invert_contract", "sequential-source-w-invert-check-smear", g_cart_id == 0 );

                }  /* end of loop on vector and axial vector */
            ...
            for ( int icoh = 0; icoh < g_coherent_source_number; icoh++ ) 
            {
              ...
              /***************************************************************************
               * we need for W
               *   p u1u u5u p type insertion into proton
               *   p u1u d5d p type insertion into proton
               *   p d1d u5u p type insertion into proton
               *   --------------------------------------
               *   n d1d d5d n type insertion into neutron 
               *   n d1d u5u n type insertion into neutron 
               *   n u1u d5d n type insertion into neutron 
               *
               *   the neutron correlators follow from teh proton case by flavor change
               *
               ***************************************************************************/
              for ( int iflavor = 0; iflavor < 2; iflavor++ ) 
              {
                ...
                for ( int if2 = 0; if2 < gamma_f1_number; if2++ ) 
                {
                  ...
                  /***************************************************************************
                   * BEGIN OF p u1u u5u p   <---> n d1d d5d n (by flavor change)
                   ***************************************************************************/
                  sprintf ( aff_tag_prefix, "/%s/w%c%c-f%c-w%c%c/T%d_X%d_Y%d_Z%d/QX%d_QY%d_QZ%d/sample%d/Gc_%s-%s",
                      correlator_tag, 
                      ...
                      isample, 
                      gamma_id_to_ascii[ sequential_gamma_id[0][ig] ],
                      gamma_id_to_ascii[ sequential_gamma_id[1][ig] ] );
                  ...
                  for ( int icol2 = 0; icol2 < 3; icol2++ ) 
                  {
                     ...
                     ***************************************************************************/
                    assign_fermion_propagator_from_spinor_field ( fp2, sequential_propagator[iflavor][0][icol1][icol2], VOLUME);

                    /***************************************************************************
                     * fp = seq up-after-up icol2, icol1
                     ***************************************************************************/
                    assign_fermion_propagator_from_spinor_field ( fp,  sequential_propagator[iflavor][1][icol2][icol1], VOLUME);
                    ...
                    if ( exitstatus != 0 ) {
                      fprintf(stderr, "[njjn_w_pv_invert_contract] Error from reduce, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
                      EXIT( 1 );
                    }
...
#pragma omp parallel for
                    ...
                    /* contraction */
                    exitstatus = contract_v6 ( vx_aux, fp2, fp3, fp, VOLUME );
                    if ( exitstatus != 0 ) {
                      fprintf(stderr, "[njjn_w_pv_invert_contract] Error from reduce, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
                      EXIT( 1 );
                    }
...
#pragma omp parallel for
                  ...
                  if ( exitstatus != 0 ) {
                    fprintf(stderr, "[njjn_w_pv_invert_contract] Error from project_write, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
                    EXIT(48);
                  }
                  ...
                  /***************************************************************************
                   * BEGIN OF p u1u d5d p   <---> n d1d u5u n (by flavor change)
                   ***************************************************************************/
                  sprintf ( aff_tag_prefix, "/%s/w%c%c-w%c%c-f%c/T%d_X%d_Y%d_Z%d/QX%d_QY%d_QZ%d/sample%d/Gc_%s-%s",
                      correlator_tag, 
                      flavor_tag[iflavor], flavor_tag[iflavor], 
                      ...
                      momentum[0], momentum[1], momentum[2],
                      isample,
                      gamma_id_to_ascii[ sequential_gamma_id[0][ig] ], gamma_id_to_ascii[ sequential_gamma_id[1][ig] ] );
                  ...
                  for ( int icol2 = 0; icol2 < 3; icol2++ ) 
                  {
                    ...
                    /***************************************************************************
                     * fp = seq dn-after-dn icol2, icol1 with gamma_5
                     ***************************************************************************/
                    assign_fermion_propagator_from_spinor_field ( fp,  sequential_propagator[1-iflavor][1][icol2][icol1], VOLUME);

                    /***************************************************************************
                     * calculate fp <- Gamma[if2] x fp x Gamma[if1]
                     ***************************************************************************/
                    ...
                    /* contraction 
                     *  T1 ( [seq up-after-up], [seq dn-after-dn], [U] ) = v5 ( fp2, fp, fp3 )
                     */
...
#pragma omp parallel for
                    ...
                    /* contraction 
                     *  T1 ( [U], [seq dn-after-dn],  [seq up-after-up] ) = v5 ( fp3, fp, fp2 )
                     */
...
#pragma omp parallel for
                    ...
                    /* contraction 
                     *  T2 ( [seq up-after-up], [seq dn-after-dn, g5], [U] ) = v6 ( fp2, fp, fp3 )
                     */
                    exitstatus = contract_v6 ( vx_aux, fp2, fp, fp3, VOLUME );
                    if ( exitstatus != 0 ) {
                      fprintf(stderr, "[njjn_w_pv_invert_contract] Error from reduce, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
                      EXIT( 1 );
                    }
...
#pragma omp parallel for
                    ...
                    /* contraction 
                     *  T2 ( [U] , [seq dn-after-dn, g5], [seq up-after-up] ) = v6 ( fp3, fp, fp2 )
                     */
                    exitstatus = contract_v6 ( vx_aux, fp3, fp, fp2, VOLUME );
                    if ( exitstatus != 0 ) {
                      fprintf(stderr, "[njjn_w_pv_invert_contract] Error from reduce, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
                      EXIT( 1 );
                    }
...
#pragma omp parallel for
                  ...
                  /***************************************************************************
                   * BEGIN OF p u5u d1d p   <---> n d5d u1u n (by flavor change)
                   ***************************************************************************/
                  sprintf ( aff_tag_prefix, "/%s/w%c%c-w%c%c-f%c/T%d_X%d_Y%d_Z%d/QX%d_QY%d_QZ%d/sample%d/Gc_%s-%s",
                      correlator_tag, 
                      flavor_tag[iflavor], flavor_tag[iflavor],
                      ...
                      momentum[0], momentum[1], momentum[2],
                      isample, 
                      gamma_id_to_ascii[ sequential_gamma_id[1][ig] ], gamma_id_to_ascii[ sequential_gamma_id[0][ig] ] );
                  ...
                  for ( int icol2 = 0; icol2 < 3; icol2++ ) 
                  {
                      ...
                      /***************************************************************************
                       * fp2 = seq up-after-up icol1, icol2 with gamma_5
                       ***************************************************************************/
                      assign_fermion_propagator_from_spinor_field ( fp2, sequential_propagator[iflavor][1][icol1][icol2], VOLUME);

                      /***************************************************************************
                       * fp = seq dn-after-dn icol2, icol1
                       ***************************************************************************/
                      assign_fermion_propagator_from_spinor_field ( fp,  sequential_propagator[1-iflavor][0][icol2][icol1], VOLUME);

                      /***************************************************************************
                       * calculate fp <- Gamma[if2] x seq dn-after-dn x Gamma[if1]
                       ***************************************************************************/
                      fermion_propagator_field_eq_gamma_ti_fermion_propagator_field ( fp, gamma_f1_list[if2], fp, VOLUME );
                      ...
                      memset ( vx_aux[0], 0, 32 * sizeof ( double ) );

                      /* contraction 
                       * T1 ( [seq up-after-up,g5], [seq dn-after-dn], [U] ) = v5 ( fp2, fp, fp3 )
                       */
                      exitstatus = contract_v5 ( vx_aux, fp2, fp, fp3, VOLUME );
                      if ( exitstatus != 0 ) {
                        fprintf(stderr, "[njjn_w_pv_invert_contract] Error from reduce, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
...
#pragma omp parallel for
                      ...
                      /* contraction 
                       * T1 ( [U] , [seq dn-after-dn], [seq up-after-up,g5] ) = v5 ( fp3, fp, fp2 )
                       */
                      exitstatus = contract_v5 ( vx_aux, fp3, fp, fp2, VOLUME );
                      if ( exitstatus != 0 ) {
                        fprintf(stderr, "[njjn_w_pv_invert_contract] Error from reduce, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
...
#pragma omp parallel for
                  ...
                  for ( int icol2 = 0; icol2 < 3; icol2++ ) 
                  {
                     ...
                     ***************************************************************************/
                    assign_fermion_propagator_from_spinor_field ( fp2, sequential_propagator[iflavor][0][icol1][icol1], VOLUME);

                    /***************************************************************************
                     * fp = seq up-after-up icol2, icol2
                     ***************************************************************************/
                    assign_fermion_propagator_from_spinor_field ( fp,  sequential_propagator[iflavor][1][icol2][icol2], VOLUME);
...
#pragma omp parallel for
                  ...
                  for ( int icol2 = 0; icol2 < 3; icol2++ ) 
                  {
                    ...
                    /***************************************************************************
                     * fp = seq dn-after-dn icol2, icol2 with gamma_5
                     ***************************************************************************/
                    assign_fermion_propagator_from_spinor_field ( fp,  sequential_propagator[1-iflavor][1][icol2][icol2], VOLUME);
...
#pragma omp parallel for
                  ...
                  for ( int icol2 = 0; icol2 < 3; icol2++ ) 
                  {
                      ...
                      /***************************************************************************
                       * fp2 = seq up-after-up icol1, icol1 with gamma_5
                       ***************************************************************************/
                      assign_fermion_propagator_from_spinor_field ( fp2, sequential_propagator[iflavor][1][icol1][icol1], VOLUME);

                      /***************************************************************************
                       * fp = seq dn-after-dn icol2, icol2
                       ***************************************************************************/
                      assign_fermion_propagator_from_spinor_field ( fp,  sequential_propagator[1-iflavor][0][icol2][icol2], VOLUME);
...
#pragma omp parallel for
                ...
                gettimeofday ( &tb, (struct timezone *)NULL );
                show_time ( &ta, &tb, "njjn_w_pv_invert_contract", "w-uuuu-diagram-reduce-project-write", g_cart_id == 0 );
    ...
    if(io_proc == 2) {
      ...
      if( aff_status_str != NULL ) {
        fprintf(stderr, "[njjn_w_pv_invert_contract] Error from aff_writer_close, status was %s %s %d\n", aff_status_str, __FILE__, __LINE__);
        EXIT(32);
      }
    }  /* end of if io_proc == 2 */
  ...
  ***************************************************************************/
  ...
  show_time ( &start_time, &end_time, "njjn_w_pv_invert_contract", "runtime", g_cart_id == 0 );
