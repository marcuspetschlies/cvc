...
void usage() {
  fprintf(stdout, "Code to perform contractions for conn. contrib. to vac. pol. with point sources\n");
...
void get_filename2(char *filename,const int sl, const int nu, const int sc, const int sign) {
  ...
  if(format==0) {
    /* format from invert */
    if(sign==1) {
      sprintf(filename, "%s.%.4d.%d.%d.%.2d.inverted", filename_prefix, Nconf,sl, nu, sc);
    }
    else {
      sprintf(filename, "%s.%.4d.%d.%d.%.2d.inverted", filename_prefix2, Nconf,sl, nu, sc);
    }
  }
} 
...
#pragma omp parallel for
  ...
  for(ia=0; ia<12; ia++) {
    if(!mms) {
      get_filename2(filename,source_location, 4, ia, 1); // position 4 is source position y, 1 means "+ i mu gamma_5", i.e. psource...
  ...
  for(nu=0; nu<4; nu++) {
    ...
    for(ia=0; ia<12; ia++) {
      if(!mms) {
        if(ud_one_file==0) {
          get_filename2(filename, source_location, nu, ia, -1);//-1 implies reading from msource..
          read_lime_spinor(cvc_spinor_field[12+ia], filename, 0);
        } else {
          get_filename2(filename, source_location,nu, ia, 1);
...
#pragma omp parallel for private(ix, spinor1, spinor2, U_, w, w1)  shared(imunu, ia, ib, nu, mu)
  ...
  for(ia=0; ia<12; ia++) {
    if(!mms) {
      if(ud_one_file==0) {
        get_filename2(filename, source_location,4, ia, -1);
        read_lime_spinor(cvc_spinor_field[12+ia], filename, 0);
      } else {
        get_filename2(filename,source_location, 4, ia, 1);
        read_lime_spinor(cvc_spinor_field[12+ia], filename, 1);
...
#pragma omp parallel for
  ...
  if(outfile_prefix_set) {
    sprintf(filename, "%s/cvc_v_x.%.4d.%.4d", outfile_prefix, Nconf, source_location);
  } else {
    sprintf(filename, "cvc_v_x.%.4d.%.4d", Nconf,source_location);
  }
  sprintf(contype, "cvc - cvc in position space, all 16 components");
  write_lime_contraction(conn, filename, 64, 16, contype, Nconf, 0);
  if(write_ascii) {
    if(outfile_prefix_set) {
      sprintf(filename, "%s/cvc_v_x.%.4d.%.4d.ascii", outfile_prefix, Nconf, source_location);
    } else {
      sprintf(filename, "cvc_v_x.%.4d.%.4d.ascii", Nconf,source_location);
    }
  ...
  if(check_position_space_WI) {
    sprintf(filename, "WI_X.%.4d.%.4d", Nconf, source_location);
...
#  endif
...
#endif
  if(outfile_prefix_set) {
    sprintf(filename, "%s/cvc_v_p.%.4d.%.4d", outfile_prefix, Nconf, source_location);
  } else {
    sprintf(filename, "cvc_v_p.%.4d.%.4d", Nconf, source_location);
  }
  sprintf(contype, "cvc - cvc in momentum space, all 16 components");
  write_lime_contraction(conn, filename, 64, 16, contype, Nconf, 0);
  if(write_ascii) {
    if(outfile_prefix_set) {
      sprintf(filename, "%s/cvc_v_p.%.4d.%.4d.ascii", outfile_prefix, Nconf, source_location);
    } else {
      sprintf(filename, "cvc_v_p.%.4d.%.4d.ascii", Nconf, source_location);
    }
  ...
  if(check_momentum_space_WI) {
    sprintf(filename, "WI_P.%.4d.%.4d", Nconf, source_location);
    ofs = fopen(filename,"w");
