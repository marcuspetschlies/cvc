 ...
 *******************************************************************/
  ...
  while ((c = getopt(argc, argv, "pqch?f:")) != -1) {
    ...
    case 'c':
      ...
      fprintf(stdout, "# [nppn_sequential] will check_propagator_residual\n");
      break;
    case 'p':
      read_forward_propagator = 1;
      fprintf(stdout, "# [nppn_sequential] will read forward propagator\n");
      break;
    case 'q':
      read_sequential_propagator = 1;
      fprintf(stdout, "# [nppn_sequential] will read sequential propagator\n");
      break;
    case 'h':
    case '?':
    default:
  ...
  if(g_fermion_type == -1 ) {
    fprintf(stderr, "# [nppn_sequential] fermion_type must be set\n");
    exit(1);
  } else {
    fprintf(stdout, "# [nppn_sequential] using fermion type %d\n", g_fermion_type);
  }

#ifdef HAVE_TMLQCD_LIBWRAPPER

  fprintf(stdout, "# [nppn_sequential] calling tmLQCD wrapper init functions\n");
...
#else
  fprintf(stdout, "[nppn_sequential] Warning, resetting global thread number to 1\n");
  g_num_threads = 1;
#endif
   ...
   ******************************************************/
  if ( g_cart_id == 0 ) {
    fprintf(stdout, "# [nppn_sequential] git version = %s\n", g_gitversion);
  }
  ...
  if(init_geometry() != 0) {
    fprintf(stderr, "[nppn_sequential] Error from init_geometry\n");
    EXIT(1);
  }
  geometry();
  ...
  switch(g_gauge_file_format) {
    ...
    case 1:
      ...
      if(g_cart_id==0) fprintf(stdout, "\n# [nppn_sequential] reading gauge field from file %s\n", filename);
      exitstatus = read_nersc_gauge_field(g_gauge_field, filename, &plaq_r);
      break;
  }
  if(exitstatus != 0) {
    fprintf(stderr, "[nppn_sequential] Error, could not read gauge field\n");
    EXIT(21);
  }
#else
  Nconf = g_tmLQCD_lat.nstore;
  if(g_cart_id== 0) fprintf(stdout, "[nppn_sequential] Nconf = %d\n", Nconf);
  ...
  if( g_gauge_field == NULL) {
    fprintf(stderr, "[nppn_sequential] Error, g_gauge_field is NULL\n");
    EXIT(5);
  }
#endif
  ...
  if ( ( exitstatus = plaquetteria  ( g_gauge_field ) ) != 0 ) {
    fprintf(stderr, "[nppn_sequential] Error from plaquetteria, status was %d\n", exitstatus);
    EXIT(2);
  }
  ...
  if( N_Jacobi > 0 ) {
    ...
    if ( N_ape > 0 ) {
      ...
      if(exitstatus != 0) {
        fprintf(stderr, "[nppn_sequential] Error from APE_Smearing, status was %d\n", exitstatus);
        EXIT(47);
      }
    }  /* end of if N_aoe > 0 */
  ...
  if ( check_propagator_residual ) {
    ...
    if(exitstatus != 0) {
      fprintf(stderr, "[nppn_sequential] Error from gauge_field_eq_gauge_field_ti_phase, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
      EXIT(38);
    }
    ...
    if ( exitstatus != 0 ) {
      fprintf(stderr, "[nppn_sequential] Error from init_clover, status was %d\n", exitstatus );
      EXIT(1);
    }
   ...
   ***********************************************************/
  double **** propagator_list = init_4level_dtable ( 2, 12, g_coherent_source_number, _GSI(VOLUME) );
  if( propagator_list == NULL ) {
    fprintf(stderr, "[nppn_sequential] Error from init_Xlevel_Ytable %s %d\n", __FILE__, __LINE__ );
    EXIT(44);
  }
  ...
  for( int i_src = 0; i_src<g_source_location_number; i_src++) {
    ...
    if(io_proc == 2) {
      sprintf(filename, "%s.%.4d.tsrc%.2d.aff", "nppn", Nconf, t_base );
      fprintf(stdout, "# [nppn_sequential] writing data to file %s\n", filename);
      affw = aff_writer(filename);
      aff_status_str = (char*)aff_writer_errstr(affw);
      if( aff_status_str != NULL ) {
        fprintf(stderr, "[nppn_sequential] Error from aff_writer, status was %s\n", aff_status_str);
        EXIT(4);
      }
    }  /* end of if io_proc == 2 */
    ...
    for( int i_coherent=0; i_coherent<g_coherent_source_number; i_coherent++) {
      ...
      for ( int iflavor = 0; iflavor <= 1; iflavor ++ ) {
        ...
        if ( read_forward_propagator ) {
          for ( int isc = 0; isc < n_s*n_c; isc++ ) {
            sprintf ( filename, "source.%c.%.4d.t%dx%dy%dz%d.%.2d.inverted", flavor_tag[iflavor], Nconf, gsx[0], gsx[1], gsx[2], gsx[3], isc );

            if ( g_cart_id == 0 && g_verbose > 2 ) fprintf ( stdout, "# [nppn_sequential] reading fwd propagator flavor %s %s %d\n", filename, __FILE__, __LINE__);

            exitstatus = read_lime_spinor ( propagator_list[iflavor][i_coherent][isc], filename, 0 );
            if(exitstatus != 0) {
              fprintf(stderr, "[nppn_sequential] Error from read_lime_spinor, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
              EXIT(12);
            }
          }
        } else {
          exitstatus = point_source_propagator ( propagator_list[iflavor][i_coherent], gsx, op_id, 1, 1, gauge_field_smeared, check_propagator_residual, gauge_field_with_phase, mzz );
          if(exitstatus != 0) {
            fprintf(stderr, "[nppn_sequential] Error from point_source_propagator, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
            EXIT(12);
          }
        }
      ...
      for ( int iflavor = 0; iflavor <= 1; iflavor ++ ) {
        ...
        assign_fermion_propagator_from_spinor_field ( fp,  propagator_list[iflavor][i_coherent], VOLUME);

        /* dn propagator as propagator type field */
        assign_fermion_propagator_from_spinor_field ( fp2, propagator_list[1-iflavor][i_coherent], VOLUME);
        ...
        for ( int if2 = 0; if2 < gamma_f1_nucleon_number; if2++ ) {
          ...
          if ( exitstatus != 0 ) {
            fprintf(stderr, "[nppn_sequential] Error from contract_v5, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
            EXIT(48);
          }

          exitstatus = contract_vn_momentum_projection ( vp, v2, 16, g_sink_momentum_list, g_sink_momentum_number);
          if ( exitstatus != 0 ) {
            fprintf(stderr, "[nppn_sequential] Error from contract_vn_momentum_projection, status was %d\n", exitstatus, __FILE__, __LINE__ );
            EXIT(48);
          }

          exitstatus = contract_vn_write_aff ( vp, 16, affw, aff_tag, g_sink_momentum_list, g_sink_momentum_number, io_proc );
          if ( exitstatus != 0 ) {
            fprintf(stderr, "[nppn_sequential] Error from contract_vn_write_aff, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
            EXIT(49);
          }
          ...
          if ( exitstatus != 0 ) {
            fprintf(stderr, "[nppn_sequential] Error from contract_v6, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
            EXIT(48);
          }

          exitstatus = contract_vn_momentum_projection ( vp, v2, 16, g_sink_momentum_list, g_sink_momentum_number);
          if ( exitstatus != 0 ) {
            fprintf(stderr, "[nppn_sequential] Error from contract_vn_momentum_projection, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
            EXIT(48);
          }
      ...
      for ( int iflavor2 = 0; iflavor2 <=1 ; iflavor2 ++ ) {
        ...
        for ( int igf2 = 0; igf2 < gamma_f2_number; igf2++ ) {
          ...
          contract_twopoint_xdep (v3[0], gamma_i2_list[igi2], gamma_f2_list[igf2], propagator_list[iflavor][i_coherent], propagator_list[iflavor2][i_coherent], 3, 1, 1., 64);
    ...
    for( int iseq_mom=0; iseq_mom < g_seq_source_momentum_number; iseq_mom++) {
       ...
       ***********************************************************/
      if(g_cart_id == 0) fprintf(stdout, "# [nppn_sequential] sequential inversion fpr pi2 = (%d, %d, %d)\n", 
        ...
        if(prop_list == NULL) {
          fprintf(stderr, "[nppn_sequential] Error from malloc\n");
          EXIT(43);
        }
        ...
        for( int is=0;is<n_s*n_c;is++) {
          ...
          if(exitstatus != 0) {
            fprintf(stderr, "[nppn_sequential] Error from init_coherent_sequential_source, status was %d\n", exitstatus);
            EXIT(14);
          }
          ...
          if(exitstatus != 0) {
            fprintf(stderr, "[nppn_sequential] Error from tmLQCD_invert, status was %d\n", exitstatus);
            EXIT(12);
          }
        ...
        retime = _GET_TIME;
        if(g_cart_id == 0) fprintf(stdout, "# [nppn_sequential] time for seq propagator = %e seconds\n", retime-ratime);
      ...
      if ( exitstatus != 0 ) {
        fprintf(stderr, "[nppn_sequential] Error from init_2level_buffer, status was %d\n", exitstatus);
        EXIT(47);
      }

      exitstatus= init_3level_buffer ( &vp, T, g_sink_momentum_number, 24 );
      if ( exitstatus != 0 ) {
        fprintf(stderr, "[nppn_sequential] Error from init_3level_buffer, status was %d\n", exitstatus);
        EXIT(47);
      }
      ...
      for ( int i = 0; i < gamma_f2_number; i++ ) {
        ...
        for ( int i_sample = 0; i_sample < g_nsample; i_sample++ ) {
          ...
          if ( exitstatus != 0 ) {
            fprintf(stderr, "[nppn_sequential] Error from contract_v3, status was %d\n", exitstatus);
            EXIT(47);
          }
          ...
          if ( exitstatus != 0 ) {
            fprintf(stderr, "[nppn_sequential] Error from contract_vn_write_aff, status was %d\n", exitstatus);
            EXIT(49);
          }
#if 0
          /*****************************************************************
      ...
      for( int i_coherent=0; i_coherent<g_coherent_source_number; i_coherent++) {
        ...
        for ( int if2 = 0; if2 < gamma_f1_delta_number; if2++ ) {
          ...
          if ( exitstatus != 0 ) {
           fprintf(stderr, "[nppn_sequential] Error from contract_v5, status was %d\n", exitstatus);
             EXIT(48);
          }
          ...
          if ( exitstatus != 0 ) {
            fprintf(stderr, "[nppn_sequential] Error from contract_v6, status was %d\n", exitstatus);
            EXIT(48);
          }
        ...
        for ( int i = 0; i < gamma_f1_nucleon_number; i++ ) {
          for ( int i_sample = 0; i_sample < g_nsample; i_sample++ ) {
            ...
            if ( exitstatus != 0 ) {
              fprintf(stderr, "[nppn_sequential] Error from contract_v1, status was %d\n", exitstatus);
              EXIT(47);
            }
            ...
            if ( exitstatus != 0 ) {
              fprintf(stderr, "[nppn_sequential] Error from contract_v2_from_v1, status was %d\n", exitstatus);
              EXIT(47);
            }
    ...
    if(io_proc == 2) {
      ...
      if( aff_status_str != NULL ) {
        fprintf(stderr, "[nppn_sequential] Error from aff_writer_close, status was %s\n", aff_status_str);
        EXIT(111);
      }
    }  /* end of if io_proc == 2 */
  ...
  for( int i_src=0; i_src < g_source_location_number; i_src++) {
    ...
    for(int i_coherent = 0; i_coherent < g_coherent_source_number; i_coherent++) {
      ...
      if ( ! read_stochastic_source_oet ) {
        ...
        if(exitstatus != 0) {
          fprintf(stderr, "[nppn_sequential] Error from init_rng_stat_file status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
          EXIT(38);
        }
      }
        ...
        if ( read_stochastic_source_oet ) {
          for ( int ispin = 0; ispin < 4; ispin++ ) {
            ...
            if ( ( exitstatus = read_lime_spinor( stochastic_source_list[ispin], filename, 0) ) != 0 ) {
              fprintf(stderr, "[nppn_sequential] Error from read_lime_spinor, status was %d\n", exitstatus);
              EXIT(2);
            }
          }
          /* recover the random field */
          if( (exitstatus = init_timeslice_source_oet(stochastic_source_list, gsx[0], NULL, -1 ) ) != 0 ) {
            fprintf(stderr, "[nppn_sequential] Error from init_timeslice_source_oet, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
            EXIT(64);
          }
          ...
          if ( write_stochastic_source_oet ) {
            for ( int ispin = 0; ispin < 4; ispin++ ) {
              ...
              if ( ( exitstatus = write_propagator( stochastic_source_list[ispin], filename, 0, 64) ) != 0 ) {
                fprintf(stderr, "[nppn_sequential] Error from write_propagator, status was %d\n", exitstatus);
                EXIT(2);
              }
            }
          }
        ...
        if ( !read_stochastic_propagator_oet ) {
          for( int i = 0; i < 4; i++) {
            ...
            if ( ( exitstatus = Jacobi_Smearing(gauge_field_smeared, spinor_work[0], N_Jacobi, kappa_Jacobi) ) != 0 ) {
              fprintf(stderr, "[nppn_sequential] Error from Jacobi_Smearing, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
              EXIT(5);
            }
          ...
          for( int i = 0; i < 4; i++) {
            ...
            if ( g_cart_id == 0 ) fprintf( stdout, "# [nppn_sequential] Reading stochastic propagator oet mom from file %s %s %d\n", filename, __FILE__, __LINE__ );
           ...
           *****************************************************************/
            ...
            for( int if1 = 0; if1 < gamma_f1_nucleon_number; if1++ ) {
              ...
              for( int ispin = 0; ispin < 4; ispin++ ) {
                ...
                exitstatus = contract_v2_from_v1 ( v2, v1, fp, VOLUME );
                if ( exitstatus != 0 ) {
                  fprintf(stderr, "[nppn_sequential] Error from contract_v4, status was %d\n", exitstatus);
                  EXIT(47);
                }
  ...
  if(g_cart_id == 0) {
    ...
    fprintf(stdout, "# [nppn_sequential] %s# [nppn_sequential] end fo run\n", ctime(&g_the_time));
    fflush(stdout);
    fprintf(stderr, "# [nppn_sequential] %s# [nppn_sequential] end fo run\n", ctime(&g_the_time));
    fflush(stderr);
  }
