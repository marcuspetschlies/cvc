          ...
          for ( int iconf = 0; iconf < num_conf; iconf++ )
          {
            for( int isrc = 0; isrc < num_src_per_conf; isrc++ )
            {
              for ( int ifbwd = 0; ifbwd < 2; ifbwd++ )
              {
                ...
                {
                  for ( int it = 0; it < T_global; it++ ) {
                    fprintf ( ofs, "%6d %3d %2d %s %2d    %4d %25.16e %25.16e\n", conf_src_list[iconf][isrc][0],
                        conf_src_list[iconf][isrc][1], fbwd_str[ifbwd], 1 - 2*ifbwd, icomp,
                        it, corr_std[iconf][isrc][ifbwd][icomp][2*it], corr_std[iconf][isrc][ifbwd][icomp][2*it+1] );
                  }
...
#pragma omp parallel for
              ...
              char obsname[200];
              sprintf ( obsname, "%s.%s_%s.%s.p%d.PX%d_PY%d_PZ%d.%s.uwerr", g_outfile_prefix,
                  gamma_id_to_Cg_ascii[gamma_f1_list[0]], gamma_id_to_Cg_ascii[gamma_f1_list[if1]], fbwd_str[ifbwd], 1 - 2 * ifbwd,
                  sink_momentum[0], sink_momentum[1], sink_momentum[2], reim_str[ireim] );

              exitstatus = apply_uwerr_real ( data[0][0], num_conf*num_src_per_conf, T_global, 0, 1, obsame );
              if ( exitstatus != 0 ) {
...
#pragma omp paralle for
          ...
          char obsname[200];
          sprintf ( obsname, "%s.avg.p%d.PX%d_PY%d_PZ%d.uwerr", g_outfile_prefix, sink_momentum[0], sink_momentum[1], sink_momentum[2] );
          ...
          if ( ireim == 0) {
            ...
            for ( int itau = 1; itau < T_global/2; itau++ )
            {
              ...
              char obsname[200];
              sprintf ( obsname, "%s.avg.acoshratio.tau%d.PX%d_PY%d_PZ%d.%s.uwer", g_outfile_prefix, itau,
                   sink_momentum[0], sink_momentum[1], sink_momentum[2], reim_str[ireim] );
