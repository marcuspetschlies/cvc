    ...
    for( int i_src = 0; i_src<g_source_location_number; i_src++) {
      ...
      for( int i_coherent=0; i_coherent<g_coherent_source_number; i_coherent++) {
        ...
        if ( twopoint_function_allocate ( &tp ) == NULL ) {
          fprintf ( stderr, "[test_correlator_read] Error from twopoint_function_allocate %s %d\n", __FILE__, __LINE__ );
          EXIT(123);
        }
        ...
        if (  tp_project == NULL ) {
          fprintf ( stderr, "[test_correlator_read] Error from malloc %s %d\n", __FILE__, __LINE__ );
          EXIT(124);
        }
        ...
        for ( int irotl = 0; irotl < 2*nrot; irotl ++ ) {
          ...
          if ( g_verbose > 3 ) fprintf ( stdout, "# [test_correlator_read] left rotref %2d - %2d\n", irotl / nrot, irotl % nrot );
          ...
          if ( gf11.id == -1 || gf12.id == -1 ) {
            fprintf ( stderr, "[test_correlator_read] Error from gamma_eq_gamma_op_ti_gamma_matrix_ti_gamma_op for gf1 %s %d\n", __FILE__, __LINE__ );
            EXIT(217);
          }
        ...
        for ( int irotr = 0; irotr < 2*nrot; irotr ++ ) {
          ...
          if ( g_verbose > 3 ) fprintf ( stdout, "# [test_correlator_read] right rotref %2d - %2d\n", irotr / nrot, irotr % nrot );
          ...
          if ( gi11.id == -1 || gi12.id == -1 ) {
            fprintf ( stderr, "[test_correlator_read] Error from gamma_eq_gamma_op_ti_gamma_matrix_ti_gamma_op for gi1 %s %d\n", __FILE__, __LINE__ );
            EXIT(218);
          }
          ...
          /* TEST */
          if ( g_verbose > 4 ) 
            fprintf ( stdout, "# [test_correlator_read] rot %2d %2d     gf11 %2d %6.2f   gf12 %2d %6.2f   gf2 %2d %6.2f   gi11 %2d %6.2f   gi12 %2d %6.2f   gi2 %2d %6.2f\n", 
          ...
          if ( ( exitstatus = twopoint_function_fill_data ( &tp, filename_prefix ) ) != 0 ) {
            fprintf ( stderr, "[test_correlator_read] Error from twopoint_function_fill_data, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
            EXIT(212);
          }
          */
          ...
          for ( int ids = 0; ids < tp.n; ids++ ) {
            if ( ( exitstatus = twopoint_function_data_location_identifier ( udli_name, &tp, filename_prefix, ids, "#" ) ) != 0 ) {
              fprintf ( stderr, "[test_correlator_read] Error from twopoint_function_data_location_identifier, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
              EXIT(212);
            }
            ...
            retime = _GET_TIME;
            if ( g_verbose > 2 ) fprintf ( stdout, "# [test_correlator_read] time for matching udli_name = %e seconds %s %d\n", retime-ratime, __FILE__, __LINE__ );

            if ( udli_id == -1 ) {
              fprintf ( stdout, "# [test_correlator_read] could not find udli_name %s in udli_list\n", udli_name );
              ...
              if ( udli_count == MAX_UDLI_NUM ) {
                fprintf ( stderr, "[test_correlator_read] Error, maximal number of udli exceeded\n" );
                EXIT(111);
              } else {
                if ( g_verbose > 2 ) fprintf ( stdout, "# [test_correlator_read] starting udli entry number %d\n", udli_count );
              }
              ...
              if ( ( exitstatus = twopoint_function_fill_data_from_udli ( udli_ptr[udli_count] , udli_name , io_proc) ) != 0 ) {
                fprintf ( stderr, "[test_correlator_read] Error from twopoint_function_fill_data_from_udli, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
                EXIT(212);
              }
              ...
              if ( g_verbose > 2 ) fprintf ( stdout, "# [test_correlator_read] udli_name %s matches udli_list[%d] %s\n", udli_name, udli_id, udli_list[udli_id] );
            }
          ...
          if ( ( exitstatus = twopoint_function_apply_diagram_norm ( &tp ) ) != 0 ) {
            fprintf ( stderr, "[test_correlator_read] Error from twopoint_function_apply_diagram_norm, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
            EXIT(213);
          }
          ...
          */
          if ( ( exitstatus = twopoint_function_accum_diagrams ( tp.c[0], &tp ) ) != 0 ) {
            fprintf ( stderr, "[test_correlator_read] Error from twopoint_function_accum_diagrams, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
            EXIT(216);
          }
          ...
          for ( int rref = 0; rref < irrep_dim; rref++ ) {
            ...
            for ( int rsrc = 0; rsrc < irrep_dim; rsrc++ ) {
              ...
              if ( g_verbose > 4 ) fprintf ( stdout, "# [test_correlator_read] zcoeff = %16.7e   %16.7e\n", creal( zcoeff ), cimag( zcoeff ) );
        ...
        for ( int itp = 0; itp < n_tp_project; itp++ ) {
          ...
          if ( g_verbose > 4 ) fprintf ( stdout, "# [test_correlator_read] correlator norm = %25.16e %25.16e\n", creal( ztmp ), cimag( ztmp ) );

          exitstatus = contract_diagram_zm4x4_field_ti_eq_co ( tp_project[itp].c[0], ztmp, tp_project[itp].T );
          if ( exitstatus != 0 ) {
            fprintf ( stderr, "[test_correlator_read] Error from contract_diagram_zm4x4_field_ti_eq_co %s %d\n", __FILE__, __LINE__ );
            EXIT(217)
          }
          ...
          if ( exitstatus != 0 ) {
            fprintf ( stderr, "[test_correlator_read] Error from twopoint_function_write_data, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
            EXIT(12);
          }
