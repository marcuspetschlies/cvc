...
#pragma omp parallel
{
  ...
  if(!(strcmp(gaugefilename_prefix,"identity")==0)) {
    ...
    if(g_cart_id==0) fprintf(stdout, "# [hvp_lma_recombine] reading gauge field from file %s\n", filename);
    read_lime_gauge_field_doubleprec(filename);
  } else {
    // initialize unit matrices
    if(g_cart_id==0) fprintf(stdout, "\n# [hvp_lma_recombine] initializing unit matrices\n");
  ...
  if(exitstatus != 0) {
    fprintf(stderr, "[hvp_lma_recombine] Error from gauge_field_eq_gauge_field_ti_phase, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(38);
  }
  ...
  if( eo_evecs_block == NULL ) {
    fprintf(stderr, "[hvp_lma_recombine] Error from init_1level_dtable %s %d\n", __FILE__, __LINE__);
    EXIT(8);
  }
  ...
  if( exitstatus != 0) {
    fprintf(stderr, "[hvp_lma_recombine] Error from deflator_read_evecs status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(8);
  }
