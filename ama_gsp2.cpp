...
int main(int argc, char **argv) {
  ...
  while ((c = getopt(argc, argv, "uch?vf:n:")) != -1) {
    ...
    case 'n':
      ...
      fprintf(stdout, "# [ama_gsp2] dimension of eigenspace set to%d\n", evecs_num);
      break;
    case 'c':
      check_eigenvectors = 1;
      fprintf(stdout, "# [ama_gsp2] check eigenvectors set to %d\n", check_eigenvectors);
      break;
    case 'u':
      unpack_spinor_field = 1;
      fprintf(stdout, "# [ama_gsp2] unpack_spinor_field set to %d\n", unpack_spinor_field);
      break;
    case 'h':
    case '?':
    default:
  ...
  if(g_cart_id==0) fprintf(stdout, "# [ama_gsp2] Reading input from file %s\n", filename);
  read_input_parser(filename);

#ifdef HAVE_TMLQCD_LIBWRAPPER

  fprintf(stdout, "# [ama_gsp2] calling tmLQCD wrapper init functions\n");
  ...
  /* initialize geometry */
  if(init_geometry() != 0) {
    fprintf(stderr, "[ama_gsp2] ERROR from init_geometry\n");
    EXIT(4);
  }
...
#ifdef HAVE_OPENMP
  if(g_cart_id == 0) fprintf(stdout, "# [ama_gsp2] setting omp number of threads to %d\n", g_num_threads);
  omp_set_num_threads(g_num_threads);
#pragma omp parallel private(nthreads,threadid)
{
  threadid = omp_get_thread_num();
  nthreads = omp_get_num_threads();
  fprintf(stdout, "# [ama_gsp2] proc%.4d thread%.4d using %d threads\n", g_cart_id, threadid, nthreads);
}
#else
  if(g_cart_id == 0) fprintf(stdout, "[ama_gsp2] Warning, resetting global thread number to 1\n");
  g_num_threads = 1;
#endif

  if (evecs_num == 0) {
    if(g_cart_id==0) fprintf(stderr, "[ama_gsp2] eigenspace dimension is 0\n");
    EXIT(5);
  }
  evecs_eval = (double*)malloc(evecs_num * sizeof(double));
  if(evecs_eval == NULL) {
    fprintf(stderr, "[ama_gsp2] Error from malloc\n");
    EXIT(6);
  }
  ...
  if(g_cart_id==0) fprintf(stdout, "# [ama_gsp2] reading gauge field from file %s\n", filename);
...
#endif
    if(status != 0) {
      fprintf(stderr, "[ama_gsp2] Error, could not read gauge field\n");
      EXIT(7);
    }

    /* measure the plaquette */
    if(g_cart_id==0) fprintf(stdout, "# [ama_gsp2] read plaquette value 1st field: %25.16e\n", plaq);
    plaquette(&plaq);
    if(g_cart_id==0) fprintf(stdout, "# [ama_gsp2] measured plaquette value 1st field: %25.16e\n", plaq);
  } else {
    if(g_cart_id == 0) fprintf(stderr, "# [ama_gsp2] need gauge field\n");
    EXIT(8);
  }
  ...
  for(ievecs = 0; ievecs<evecs_num; ievecs+=2) {
    ...
    if(g_cart_id == 0) fprintf(stdout, "# [ama_gsp2] reading C_oo_sym eigenvector from file %s\n", filename);

    status = read_lime_spinor(full_spinor_field[0], filename, 0);
    if( status != 0) {
      fprintf(stderr, "[ama_gsp2] Error from read_lime_spinor, status was %d\n", status);
      EXIT(9);
    }
  ...
  if (check_eigenvectors) {
    ...
    for(ievecs = 0; ievecs<evecs_num; ievecs++) {
      ...
      if(g_cart_id == 0) {
        fprintf(stdout, "# [ama_gsp2] estimated eigenvalue(%d) = %25.16e\n", ievecs, evecs_lambda);
      }
      ...
      if(g_cart_id == 0) {
        fprintf(stdout, "# [ama_gsp2] eigenvector(%d) || A x - lambda x || = %16.7e\n", ievecs, sqrt(norm) );
      }
  ...
  if(g_cart_id == 0) fprintf(stdout, "# [ama_gsp2] calculating %s\n", gsp_tag);
  /* scalar products */
  ...
  if(status != 0) {
    fprintf(stderr, "[ama_gsp2] Error from gsp_calculate_v_dag_gamma_p_w, status was %d\n", status);
    EXIT(11);
  }
#
  /***********************************************
  ...
  for(ievecs = 0; ievecs<evecs_num; ievecs++) {
    ...
    retime = _GET_TIME;
    if(g_cart_id == 0) fprintf(stdout, "# [ama_gsp2] time for X_eo = %e seconds\n", retime-ratime);
  }
  /* name tag */
  ...
  for(ievecs = 0; ievecs<evecs_num; ievecs++) {
    ...
    retime = _GET_TIME;
    if(g_cart_id == 0) fprintf(stdout, "# [ama_gsp2] time for C_from_Xeo = %e seconds\n", retime-ratime);
    /* TEST */
    if(g_cart_id == 0) fprintf(stdout, "# [ama_gsp2] eval %4d %25.16e\n", ievecs, evecs_eval[ievecs]);
  }
  /* write eigenvalues */
  sprintf(gsp_tag, "%s.%.4d", "gsp_eval", Nconf);
  status = gsp_write_eval(evecs_eval, evecs_num, gsp_tag);
  if(status != 0) {
    fprintf(stderr, "[ama_gsp2] Error from gsp_write, status was %d\n", status);
    EXIT(14);
  }
  ...
  if (g_cart_id == 0) {
    ...
    fprintf(stdout, "# [ama_gsp2] %s# [ama_gsp2] end fo run\n", ctime(&g_the_time));
    fflush(stdout);
    fprintf(stderr, "# [ama_gsp2] %s# [ama_gsp2] end fo run\n", ctime(&g_the_time));
    fflush(stderr);
  }
