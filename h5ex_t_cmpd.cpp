...
int main (void) {
  ...
  complex *wdata, /* Write buffer */
          *rdata; /* Read buffer */

  double _Complex wbuffer[DIM0],                /* Read buffer */
                  rbuffer[DIM0];                /* Read buffer */
   ...
   */
  memtype = H5Tcreate (H5T_COMPOUND, 2*sizeof (double));
   ...
   *
   * we use H5T_IEEE_F64BE, ich IEEE floating point 64 bit, big endian
   */
  filetype = H5Tcreate ( H5T_COMPOUND, 2 * sizeof (double)  );
  ...
  rdata = (sensor_t *) malloc (dims[0] * 2 * sizeof (double));
