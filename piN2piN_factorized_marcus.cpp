...
/***********************************************************/
  ...
  int io_proc = -1;
  double **propagator_list_up = NULL, **propagator_list_dn = NULL, **sequential_propagator_list = NULL, **stochastic_propagator_list = NULL, **stochastic_propagator_zero_list_up = NULL,**stochastic_propagator_zero_list_dn = NULL,
 ...
 *                                                                 */
  ...
  int gamma_f1_delta_list[gamma_f1_delta_number]        = { 9,  0, 7 };
 ...
 *******************************************************************/
  ...
  for(int isample = 0; isample < g_nsample; isample++)
  {
    ...
    if ( read_stochastic_source ) {
      ...
      printf("DEBUG just after reading %s %e\n", filename, stochastic_source_list[isample][0]);
    } else {
    ...
    if ( read_stochastic_propagator ) {
      ...
      printf("Debug reading propagator %s %e\n", filename, stochastic_propagator_list[isample][0]);
    } else {
  ...
  for( int i_src = 0; i_src<g_source_location_number; i_src++) {
    ...
    for( int i_coherent=0; i_coherent<g_coherent_source_number; i_coherent++) {
      ...
      for ( int if2 = 0; if2 < gamma_f1_nucleon_number; if2++ ) {
        ...
        fermion_propagator_field_eq_gamma_ti_fermion_propagator_field ( fp3, gamma_f1_nucleon_list[if2], fp, VOLUME );
        ...
        sprintf(aff_tag, "/N-N/t%.2dx%.2dy%.2dz%.2d/gi%.2d/gf%.2d/n3",
            gsx[0], gsx[1], gsx[2], gsx[3],
            gamma_f1_nucleon_list[if1], gamma_f1_nucleon_list[if2]);

        exitstatus = contract_v5 ( v2, fp2, fp3, fp2, VOLUME );
        if ( exitstatus != 0 ) {
          fprintf(stderr, "[piN2piN_factorized] Error from contract_v5, status was %d\n", exitstatus);
          EXIT(48);
        }
        ...
        sprintf(aff_tag, "/N-N/t%.2dx%.2dy%.2dz%.2d/gi%.2d/gf%.2d/n4",
            gsx[0], gsx[1], gsx[2], gsx[3],
            gamma_f1_nucleon_list[if1], gamma_f1_nucleon_list[if2]);

        exitstatus = contract_v6 ( v2, fp2, fp3, fp, VOLUME );
        if ( exitstatus != 0 ) {
          fprintf(stderr, "[piN2piN_factorized] Error from contract_v6, status was %d\n", exitstatus);
          EXIT(48);
        }
      ...
      for ( int i = 0; i < gamma_f1_nucleon_number; i++ ) {
        ...
        for ( int i_sample = 0; i_sample < g_nsample; i_sample++ ) {
           ...
           * (5) xi - g5 gf1 - u - d
           *****************************************************************/
          sprintf(aff_tag, "/v2/t%.2dx%.2dy%.2dz%.2d/g5.xi-g%.2d-u-d/sample%.2d", gsx[0], gsx[1], gsx[2], gsx[3], gamma_f1_nucleon_list[i], i_sample);
           ...
           * xi - gf1 - d - u
           *****************************************************************/
          sprintf(aff_tag, "/v2/t%.2dx%.2dy%.2dz%.2d/g5.xi-g%.2d-d-u/sample%.2d", gsx[0], gsx[1], gsx[2], gsx[3], gamma_f1_nucleon_list[i], i_sample);
           ...
           * xi - gf1 - d - d
           *****************************************************************/
          sprintf(aff_tag, "/v2/t%.2dx%.2dy%.2dz%.2d/g5.xi-g%.2d-d-d/sample%.2d", gsx[0], gsx[1], gsx[2], gsx[3], gamma_f1_nucleon_list[i], i_sample);
    ...
    for( int iseq_mom=0; iseq_mom < g_seq_source_momentum_number; iseq_mom++) {
      ...
      for ( int i = 0; i < gamma_f2_number; i++ ) {
        ...
        for ( int i_sample = 0; i_sample < g_nsample; i_sample++ ) {
          ...
          /* spinor_work0 <- Gamma_f2^+ spinor_work1 propagator */
          spinor_field_eq_gamma_ti_spinor_field (spinor_work[0], gamma_f2_list[i], spinor_work[1], VOLUME );
      ...
      for( int i_coherent=0; i_coherent<g_coherent_source_number; i_coherent++) {
        ...
        for ( int i = 0; i < gamma_f1_nucleon_number; i++ ) {
          for ( int i_sample = 0; i_sample < g_nsample; i_sample++ ) {
             ...
             * (12) g5.xi - gf1 - ud - d
             *****************************************************************/
            sprintf(aff_tag, "/v2/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/g5.xi-g%.2d-ud-d/sample%.2d", gsx[0], gsx[1], gsx[2], gsx[3],
                g_seq_source_momentum_list[iseq_mom][0], g_seq_source_momentum_list[iseq_mom][1], g_seq_source_momentum_list[iseq_mom][2],
                gamma_f1_nucleon_list[i], i_sample);
             ...
             *****************************************************************/
             ...
             sprintf(aff_tag, "/v4/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/g5.xi-g%.2d-d-ud/sample%.2d", gsx[0], gsx[1], gsx[2], gsx[3],
                g_seq_source_momentum_list[iseq_mom][0], g_seq_source_momentum_list[iseq_mom][1], g_seq_source_momentum_list[iseq_mom][2],
                gamma_f1_nucleon_list[i], i_sample);
      ...
      if ( read_sequential_propagator ) {
        for ( int isc = 0; isc < n_s * n_c; isc++ ) {
          sprintf ( filename, "source.%c-%c.%.4d.t%dx%dy%dz%d.gseq%d.px%dpy%dpz%d.%.2d.inverted", 'u', 'u', Nconf, gsx[0], gsx[1], gsx[2], gsx[3], 5,
              g_seq_source_momentum_list[iseq_mom][0], g_seq_source_momentum_list[iseq_mom][1], g_seq_source_momentum_list[iseq_mom][2], isc );
        ...
        for( int is=0;is<n_s*n_c;is++) {
          ...
          /* extract spin-color source-component is from coherent source dn propagators */
          for( int i=0; i<g_coherent_source_number; i++) {
            if(g_cart_id == 0) fprintf(stdout, "# [piN2piN_factorized] using up prop id %d / %d\n", (i_src * g_coherent_source_number + i), (i_src * g_coherent_source_number + i)*n_s*n_c + is);
            prop_list[i] = propagator_list_up[i * n_s*n_c + is];
          }
      ...
      for ( int i = 0; i < gamma_f2_number; i++ ) {
        ...
        for ( int i_sample = 0; i_sample < g_nsample; i_sample++ ) {
          ...
          sprintf(aff_tag, "/v3/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/xi-g%.2d-uu/sample%.2d",
              g_source_coords_list[i_src][0],
              g_source_coords_list[i_src][1],
              g_source_coords_list[i_src][2],
              g_source_coords_list[i_src][3],
          ...
          sprintf(aff_tag, "/v3/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/g5.phi-g%.2d-uu/sample%.2d",
              g_source_coords_list[i_src][0],
              g_source_coords_list[i_src][1],
              g_source_coords_list[i_src][2],
              g_source_coords_list[i_src][3],
      ...
      for( int i_coherent=0; i_coherent<g_coherent_source_number; i_coherent++) {
        ...
        for ( int i = 0; i < gamma_f1_nucleon_number; i++ ) {
          for ( int i_sample = 0; i_sample < g_nsample; i_sample++ ) {
            ...
            sprintf(aff_tag, "/v2/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/g5.xi-g%.2d-uu-u/sample%.2d", gsx[0], gsx[1], gsx[2], gsx[3],
              g_seq_source_momentum_list[iseq_mom][0], g_seq_source_momentum_list[iseq_mom][1], g_seq_source_momentum_list[iseq_mom][2],
              gamma_f1_nucleon_list[i], i_sample);
            printf("DEBUG V2 %s\n", aff_tag);
            ...
            sprintf(aff_tag, "/v2/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/g5.xi-g%.2d-u-uu/sample%.2d", gsx[0], gsx[1], gsx[2], gsx[3],
              g_seq_source_momentum_list[iseq_mom][0], g_seq_source_momentum_list[iseq_mom][1], g_seq_source_momentum_list[iseq_mom][2],
              gamma_f1_nucleon_list[i], i_sample);
            ...
            sprintf(aff_tag, "/v4/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/phi-g%.2d-d-uu/sample%.2d", gsx[0], gsx[1], gsx[2], gsx[3],
                g_seq_source_momentum_list[iseq_mom][0], g_seq_source_momentum_list[iseq_mom][1], g_seq_source_momentum_list[iseq_mom][2],
                gamma_f1_nucleon_list[i], i_sample);
            ...
            sprintf(aff_tag, "/v2/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/phi-g%.2d-d-uu/sample%.2d", gsx[0], gsx[1], gsx[2], gsx[3],
              g_seq_source_momentum_list[iseq_mom][0], g_seq_source_momentum_list[iseq_mom][1], g_seq_source_momentum_list[iseq_mom][2],
              gamma_f1_nucleon_list[i], i_sample);
      ...
      if ( read_sequential_propagator ) {
        for ( int isc = 0; isc < n_s * n_c; isc++ ) {
          sprintf ( filename, "source.%c-%c.%.4d.t%dx%dy%dz%d.gseq%d.px%dpy%dpz%d.%.2d.inverted", 'd', 'd', Nconf, gsx[0], gsx[1], gsx[2], gsx[3], 5,
              g_seq_source_momentum_list[iseq_mom][0], g_seq_source_momentum_list[iseq_mom][1], g_seq_source_momentum_list[iseq_mom][2], isc );
      ...
      for ( int i = 0; i < gamma_f2_number; i++ ) {
        ...
        for ( int i_sample = 0; i_sample < g_nsample; i_sample++ ) {
          ...
          sprintf(aff_tag, "/v3/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/g5.phi-g%.2d-dd/sample%.2d",
              g_source_coords_list[i_src][0],
              g_source_coords_list[i_src][1],
              g_source_coords_list[i_src][2],
              g_source_coords_list[i_src][3],
      ...
      for( int i_coherent=0; i_coherent<g_coherent_source_number; i_coherent++) {
        ...
        for ( int i = 0; i < gamma_f1_nucleon_number; i++ ) {
          for ( int i_sample = 0; i_sample < g_nsample; i_sample++ ) {
            ...
            sprintf(aff_tag, "/v2/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/g5.xi-g%.2d-u-dd/sample%.2d", gsx[0], gsx[1], gsx[2], gsx[3],
              g_seq_source_momentum_list[iseq_mom][0], g_seq_source_momentum_list[iseq_mom][1], g_seq_source_momentum_list[iseq_mom][2],
              gamma_f1_nucleon_list[i], i_sample);
            ...
            sprintf(aff_tag, "/v4/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/g5.xi-g%.2d-u-dd/sample%.2d", gsx[0], gsx[1], gsx[2], gsx[3],
                g_seq_source_momentum_list[iseq_mom][0], g_seq_source_momentum_list[iseq_mom][1], g_seq_source_momentum_list[iseq_mom][2],
                gamma_f1_nucleon_list[i], i_sample);

            exitstatus = contract_v4 ( v2, spinor_work[1], fp4,fp, VOLUME );
            if ( exitstatus != 0 ) {
              fprintf(stderr, "[piN2piN_factorized] Error from contract_v4, status was %d\n", exitstatus);
              EXIT(47);
            }
            ...
            sprintf(aff_tag, "/v2/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/phi-g%.2d-dd-u/sample%.2d", gsx[0], gsx[1], gsx[2], gsx[3],
              g_seq_source_momentum_list[iseq_mom][0], g_seq_source_momentum_list[iseq_mom][1], g_seq_source_momentum_list[iseq_mom][2],
              gamma_f1_nucleon_list[i], i_sample);
  ...
  exitstatus = init_2level_buffer ( &stochastic_propagator_zero_list_up, 4, _GSI(VOLUME) );
  if ( exitstatus != 0 ) {
    fprintf(stderr, "[piN2piN_factorized] Error from init_2level_buffer, status was %d\n", exitstatus);
    EXIT(48);
  }

  exitstatus = init_2level_buffer ( &stochastic_propagator_zero_list_dn, 4, _GSI(VOLUME) );
  if ( exitstatus != 0 ) {
    fprintf(stderr, "[piN2piN_factorized] Error from init_2level_buffer, status was %d\n", exitstatus);
    EXIT(48);
  }
...
*/
      ...
      for( int isample=0; isample < g_nsample_oet; isample++) {
        ...
        if ( !read_stochastic_propagator_oet ) {
          for( int i = 0; i < 4; i++) {
            ...
            memcpy( stochastic_propagator_zero_list_up[i], spinor_work[1], sizeof_spinor_field);
          }

        } else {
          for( int i = 0; i < 4; i++) {
            /* sprintf(filename, "%s-oet.%c.%.4d.t%.2d.%.2d.%.5d", filename_prefix2, 'u', Nconf, gsx[0], i, isample); */
            sprintf(filename, "%s-oet.%c.%.4d.t%.2d.px%dpy%dpz%d.%.2d.%.5d", filename_prefix2, 'u', Nconf, gsx[0], 0, 0, 0, i, isample );
            if ( g_cart_id == 0 ) fprintf( stdout, "# [piN2piN_factorized] Reading stochastic propagator oet zero from file %s %s %d\n", filename, __FILE__, __LINE__ );
            exitstatus = read_lime_spinor ( stochastic_propagator_zero_list_up[i], filename, 0 );
            if ( exitstatus != 0 ) {
              fprintf(stderr, "[piN2piN_factorized] Error from read_lime_spinor, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
              EXIT(47);
            }
          ...
          for( int i = 0; i < 4; i++) {
            /* sprintf(filename, "%s-oet.%c.%.4d.t%.2d.%.2d.%.5d", filename_prefix2, 'u', Nconf, gsx[0], i, isample); */
            sprintf(filename, "%s-oet.%c.%.4d.t%.2d.px%dpy%dpz%d.%.2d.%.5d", filename_prefix2, 'd', Nconf, gsx[0], 0, 0, 0, i, isample );
            if ( g_cart_id == 0 ) fprintf( stdout, "# [piN2piN_factorized] Reading stochastic propagator oet zero from file %s %s %d\n", filename, __FILE__, __LINE__ );
            exitstatus = read_lime_spinor ( stochastic_propagator_zero_list_dn[i], filename, 0 );
            if ( exitstatus != 0 ) {
              fprintf(stderr, "[piN2piN_factorized] Error from read_lime_spinor, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
              EXIT(47);
            }
        ...
        for( int if1 = 0; if1 < gamma_f1_nucleon_number; if1++ ) {
          for( int ispin = 0; ispin < 4; ispin++ ) {
            ...
            spinor_field_eq_gamma_ti_spinor_field (spinor_work[0], gamma_f1_nucleon_list[if1], stochastic_propagator_zero_list_up[ispin], VOLUME );
             ...
             * (5) phi - gf1 - d - d
             *****************************************************************/
            sprintf(aff_tag, "/v2-oet/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/phi-g%.2d-d-d/sample%.2d/d%d", gsx[0], gsx[1], gsx[2], gsx[3],
                0, 0, 0, gamma_f1_nucleon_list[if1], isample, ispin);
            ...
            /* spinor_work <- Gamma_f1^t x stochastic propagator */
            spinor_field_eq_gamma_ti_spinor_field (spinor_work[0], gamma_f1_nucleon_list[if1], stochastic_propagator_zero_list_dn[ispin], VOLUME );
            spinor_field_ti_eq_re ( spinor_work[0], gamma_f1_nucleon_transposed_sign[if1], VOLUME);
             ...
             * (6) hat phi - gf1 - u - u
             *****************************************************************/
            sprintf(aff_tag, "/v2-oet/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/hatphi-g%.2d-u-u/sample%.2d/d%d", gsx[0], gsx[1], gsx[2], gsx[3],
                0, 0, 0, gamma_f1_nucleon_list[if1], isample, ispin);
             ...
             * (7) hat phi - gf1 - u - d
             *****************************************************************/
            sprintf(aff_tag, "/v2-oet/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/hatphi-g%.2d-u-d/sample%.2d/d%d", gsx[0], gsx[1], gsx[2], gsx[3],
                0, 0, 0, gamma_f1_nucleon_list[if1], isample, ispin);
          ...
          for( int ispin = 0; ispin < 4; ispin++ ) {
            ...
            exitstatus = contract_v4 ( v2, stochastic_propagator_zero_list_up[ispin], fp3, fp, VOLUME );
            if ( exitstatus != 0 ) {
              fprintf(stderr, "[piN2piN_factorized] Error from contract_v4, status was %d\n", exitstatus);
              EXIT(47);
            }
          ...
          /* fp3 <- Gamma_f1 x fp2 = Gamma_f1 x U */
          ...
          for( int ispin = 0; ispin < 4; ispin++ ) {
            ...
            exitstatus = contract_v4 ( v2, stochastic_propagator_zero_list_dn[ispin], fp3, fp2, VOLUME );
            if ( exitstatus != 0 ) {
              fprintf(stderr, "[piN2piN_factorized] Error from contract_v4, status was %d\n", exitstatus);
              EXIT(47);
            }
        ...
        for ( int igi2 = 0; igi2 < gamma_i2_number; igi2++ ) {
          ...
          sprintf(aff_tag, "/m-m-zero-ppuu/t%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/sample%.2d/gi%.2d/gf%.2d", gsx[0], 0, 0, 0, isample, gamma_i2_list[igi2], gamma_f2_list[igf2] );
          ...
          contract_twopoint_xdep (v3[0], 5, 5, stochastic_propagator_zero_list_up, stochastic_propagator_zero_list_up, 1, 1, 1., 64);
          ...
          sprintf(aff_tag, "/m-m-zero-ppdd/t%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/sample%.2d/gi%.2d/gf%.2d", gsx[0], 0, 0, 0, isample, gamma_i2_list[igi2], gamma_f2_list[igf2] );

          /* contract_twopoint_xdep (v3[0], gamma_i2_list[igi2], gamma_f2_list[igf2], stochastic_propagator_zero_list, stochastic_propagator_zero_list, 1, 1, 1., 64); */

          /* TEST PLEGMA COMPARISON */
          /* use gi2 = gf2 = 5 to cancel the oet g5 implicit in contract_twopoint_xdep */
          contract_twopoint_xdep (v3[0], 5, 5, stochastic_propagator_zero_list_dn, stochastic_propagator_zero_list_dn, 1, 1, 1., 64);
          ...
          sprintf(aff_tag, "/m-m-zero-p0du/t%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/sample%.2d/gi%.2d/gf%.2d", gsx[0], 0, 0, 0, isample, gamma_i2_list[igi2], gamma_f2_list[igf2] );

          /* contract_twopoint_xdep (v3[0], gamma_i2_list[igi2], gamma_f2_list[igf2], stochastic_propagator_zero_list, stochastic_propagator_zero_list, 1, 1, 1., 64); */

          /* TEST PLEGMA COMPARISON */
          /* use gi2 = gf2 = 5 to cancel the oet g5 implicit in contract_twopoint_xdep */
          contract_twopoint_xdep (v3[0], 5, 5, stochastic_propagator_zero_list_dn, stochastic_propagator_zero_list_up, 1, 1, 1., 64);
          ...
          sprintf(aff_tag, "/m-m-zero-p0ud/t%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/sample%.2d/gi%.2d/gf%.2d", gsx[0], 0, 0, 0, isample, gamma_i2_list[igi2], gamma_f2_list[igf2] );

          /* contract_twopoint_xdep (v3[0], gamma_i2_list[igi2], gamma_f2_list[igf2], stochastic_propagator_zero_list, stochastic_propagator_zero_list, 1, 1, 1., 64); */

          /* TEST PLEGMA COMPARISON */
          /* use gi2 = gf2 = 5 to cancel the oet g5 implicit in contract_twopoint_xdep */
          contract_twopoint_xdep (v3[0], 5, 5, stochastic_propagator_zero_list_up, stochastic_propagator_zero_list_dn, 1, 1, 1., 64);
        ...
        for( int iseq_mom=0; iseq_mom < g_seq_source_momentum_number; iseq_mom++) {
          ...
          for ( int igi2 = 0; igi2 < gamma_i2_number; igi2++ ) {
            ...
            /* TEST PLEGMA COMPARISON */
            /* use gi2 = gf2 = 5 to cancel the oet g5 implicit in contract_twopoint_xdep */
            contract_twopoint_xdep (v3[0], 5, 5, stochastic_propagator_list, stochastic_propagator_zero_list_up, 1, 1, 1., 64);
          ...
          for ( int if2 = 0; if2 < gamma_f2_number; if2++ ) {
            ...
            for( int ispin = 0; ispin < 4; ispin++ ) {
              ...
              /* spinor_work[1] = g5 stochastic_propagator_list */
              /* spinor_field_eq_gamma_ti_spinor_field (spinor_work[1], 5, stochastic_propagator_list[ispin], VOLUME ); */
              /* TEST PLEGMA COMPARISON */
              spinor_field_eq_gamma_ti_spinor_field (spinor_work[1], 5, stochastic_propagator_list[ispin], VOLUME );
              spinor_field_ti_eq_re ( spinor_work[1], -1., VOLUME );

              /* spinor_work[0] <- Gamma_f2^+ x spinor_work[1], up to sign */
              spinor_field_eq_gamma_ti_spinor_field (spinor_work[0], gamma_f2_list[if2], spinor_work[1], VOLUME );
              /* spinor_work <- spinor_work x sign for Gamma_f2^+ */
              spinor_field_ti_eq_re ( spinor_work[0], gamma_f2_g5_adjoint_sign[if2], VOLUME);
              ...
              /* spinor_field_eq_gamma_ti_spinor_field (spinor_work[1], 5, stochastic_propagator_list[ispin], VOLUME ); */
              /* TEST PLEGMA COMPARISON */
              spinor_field_eq_gamma_ti_spinor_field (spinor_work[1], 5, stochastic_propagator_zero_list_dn[ispin], VOLUME );
              spinor_field_ti_eq_re ( spinor_work[1], -1., VOLUME );
              ...
              sprintf(aff_tag, "/v3-oet/t%.2dx%.2dy%.2dz%.2d/pi2x%.2dpi2y%.2dpi2z%.2d/g5.hatphi-g%.2d-u/sample%.2d/d%d", gsx[0], gsx[1], gsx[2], gsx[3],
                  /* -g_seq_source_momentum_list[iseq_mom][0], -g_seq_source_momentum_list[iseq_mom][1], -g_seq_source_momentum_list[iseq_mom][2], */
                  g_seq_source_momentum_list[iseq_mom][0], g_seq_source_momentum_list[iseq_mom][1], g_seq_source_momentum_list[iseq_mom][2],
                  gamma_f2_list[if2], isample, ispin);
  ...
  fini_2level_buffer ( &stochastic_propagator_list );
  fini_2level_buffer ( &stochastic_propagator_zero_list_up );
  fini_2level_buffer ( &stochastic_propagator_zero_list_dn );
  fini_2level_buffer ( &stochastic_source_list );
  fini_2level_buffer ( &propagator_list_up );
  fini_2level_buffer ( &propagator_list_dn );
