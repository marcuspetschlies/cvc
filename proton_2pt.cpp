...
int main(int argc, char **argv) {
  ...
  if(g_fermion_type == -1 ) {
    ...
    fprintf(stdout, "# [proton_2pt] fermion_type set to %d\n", g_fermion_type);
  }
...
#pragma omp for
 ...
 free_sp(&sp2);
...
#if 0
  {
    ...
    spinor_propagator_type **conn_X = (spinor_propagator_type**)malloc(2 * sizeof(spinor_propagator_type*));
    for(i=0; i<2; i++) {
      conn_X[i] = create_sp_field( (size_t)VOLUME);
      if(conn_X[i] == NULL) {
    ...
    exitstatus = contract_N_N (conn_X, g_spinor_field, &(g_spinor_field[dn_prop_id]) );

    for(i=0; i<2; i++) { 
      exitstatus = add_baryon_boundary_phase (conn_X[i], gsx[0], 1);
    }
    ...
    fclose(ofs);
    for(i=0; i<2; i++) { free_sp_field(&conn_X[i]); }
  }  /* end of TEST */
#endif
