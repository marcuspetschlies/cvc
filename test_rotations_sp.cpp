...
#pragma omp parallel
{
  ...
  // for(int irot = 0; irot < 1; irot++ )
  {
    ...
    sprintf(name, "sp%.2d", irot );
    printf_sp( sp1, name, ofs);

    rot_bispinor_mat_ti_sp( sp2, ASpin, sp1 );

    sprintf(name, "rot_sp%.2d", irot );
    printf_sp( sp2, name, ofs);

    rot_sp_ti_bispinor_mat ( sp2, ASpin, sp1 );

    sprintf(name, "sp_rot%.2d", irot );
    printf_sp( sp2, name, ofs);
