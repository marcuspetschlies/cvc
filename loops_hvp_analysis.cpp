/****************************************************
 * loops_hvp_analysis.cpp
 *
 * 
 *
 * - originally copied from loops_hvp.cpp
 *
 * PURPOSE:
 * DONE:
 * TODO:
...
int main(int argc, char **argv) {
  ...
  double _Complex ***cvc_loop_lma = NULL, ****cvc_loop_stoch_aux = NULL, ****cvc_loop_stoch_cum = NULL;
  ...
  while ((c = getopt(argc, argv, "cWh?f:p:n:")) != -1) {
    switch (c) {
    case 'f':
  ...
  /* fprintf(stdout, "# [loops_hvp_analysis] Reading input from file %s\n", filename); */
   ...
   ******************************************************/
  if ( g_cart_id == 0 ) {
    fprintf(stdout, "# [loops_hvp_analysis] git version = %s\n", g_gitversion);
  }
   ...
   *********************************/
#ifdef HAVE_OPENMP
  if(g_cart_id == 0) fprintf(stdout, "# [loops_hvp_analysis] setting omp number of threads to %d\n", g_num_threads);
  omp_set_num_threads(g_num_threads);
#pragma omp parallel
{
  fprintf(stdout, "# [loops_hvp_analysis] proc%.4d thread%.4d using %d threads\n", g_cart_id, omp_get_thread_num(), omp_get_num_threads());
}
#else
  if(g_cart_id == 0) fprintf(stdout, "[loops_hvp_analysis] Warning, resetting global thread number to 1\n");
  g_num_threads = 1;
#endif

  if(init_geometry() != 0) {
    fprintf(stderr, "[loops_hvp_analysis] Error from init_geometry %s %d\n", __FILE__, __LINE__);
    EXIT(4);
  }
  ...
  if( g_proc_coords[0] == 0 && g_proc_coords[1] == 0 && g_proc_coords[2] == 0 && g_proc_coords[3] == 0) {
    io_proc = 2;
    fprintf(stdout, "# [loops_hvp_analysis] proc%.4d is io process\n", g_cart_id);
  } else {
    if( g_proc_coords[1] == 0 && g_proc_coords[2] == 0 && g_proc_coords[3] == 0) {
      io_proc = 1;
      fprintf(stdout, "# [loops_hvp_analysis] proc%.4d is send process\n", g_cart_id);
    } else {
      io_proc = 0;
    }
  }
  ...
  if(io_proc == 2) {
    if(g_tr_id != 0) {
      fprintf(stderr, "[loops_hvp_analysis] Error, io proc must be id 0 in g_tr_comm %s %d\n", __FILE__, __LINE__);
      EXIT(14);
    }
  }
#endif
  ...
  if ( ( exitstatus = init_rng_stat_file (g_seed, NULL) ) != 0 ) {
    fprintf(stderr, "[loops_hvp_analysis] Error from init_rng_stat_file status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(38);
  }
  ...
  if(io_proc == 2) {
    ...
    fprintf(stdout, "# [loops_hvp_analysis] reading data from file %s\n", filename);
    affr = aff_reader(filename);
    aff_status_str = (char*)aff_reader_errstr( affr );
    if( aff_status_str != NULL ) {
      fprintf(stderr, "[loops_hvp_analysis] Error from aff_reader, status was %s %s %d\n", aff_status_str, __FILE__, __LINE__);
      EXIT(15);
    }
    if( (affn = aff_reader_root(affr)) == NULL ) {
      fprintf(stderr, "[loops_hvp_analysis] Error, aff reader is not initialized %s %d\n", __FILE__, __LINE__);
      EXIT(1);
    }
  }  /* end of if io_proc == 2 */
#endif
  ...
  if( ( exitstatus = init_3level_zbuffer ( &cvc_loop_lma, g_sink_momentum_number, 4, T_global  ) ) != 0 ) {
    fprintf(stderr, "[loops_hvp_analysis] Error from init_3level_buffer, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(1);
  }
  ...
  if( ( exitstatus = init_4level_zbuffer ( &cvc_loop_stoch_aux, g_sink_momentum_number, 4, g_nsample, T_global  ) ) != 0 ) {
    fprintf(stderr, "[loops_hvp_analysis] Error from init_4level_buffer, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(1);
  }
  ...
  for ( int imom = 0; imom < g_sink_momentum_number; imom++ ) {
    ...
    for ( int mu = 0; mu < 4; mu++ ) {
      sprintf(aff_tag, "/loop/cvc/nev%.4d/px%.2dpy%.2dpz%.2d/mu%d", evecs_num, 
      ...
      exitstatus = aff_node_get_complex (affr, affdir, cvc_loop_lma[imom][mu], (uint32_t)T_global );
      if(exitstatus != 0 ) {
        fprintf(stderr, "[loops_hvp_analysis] Error from aff_node_get_complex, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
        EXIT(2);
      }
    }
    ...
    for ( int isample = 0; isample < g_nsample; isample++ ) {
      ...
      if ( zbuffer == NULL ) {
        fprintf(stderr, "[loops_hvp_analysis] Error from malloc %s %d\n", __FILE__, __LINE__);
        EXIT(1);
      }
      ...
      for ( int mu = 0; mu < 4; mu++ ) {
        ...
        for ( int t_src = 0; t_src < T_global; t_src++ ) {
          ...
          sprintf(aff_tag, "/loop/cvc/sample%.4d/tsrc%.2d/px%.2dpy%.2dpz%.2d/mu%d", isample, t_src, 
              g_sink_momentum_list[imom][0], g_sink_momentum_list[imom][1], g_sink_momentum_list[imom][2], mu );
          ...
          complex_field_pl_eq_complex_field ( (double*)cvc_loop_stoch_aux[imom][mu][isample], (double*)zbuffer, T_global );
        }
  ...
  if( ( exitstatus = init_4level_zbuffer ( &cvc_loop_stoch_cum, g_sink_momentum_number, 4, g_nsample, T_global  ) ) != 0 ) {
    fprintf(stderr, "[loops_hvp_analysis] Error from init_4level_buffer, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(1);
  }
  /***********************************************************
  ...
  for ( int imom = 0; imom < g_sink_momentum_number; imom++ ) {
    ...
    for ( int mu = 0; mu < 4; mu++ ) {
      ...
      complex_field_pl_eq_complex_field ( (double*)cvc_loop_stoch_cum[imom][mu][0], (double*)cvc_loop_stoch_aux[imom][mu][0], T_global );
      
      for ( int isample = 1; isample < g_nsample; isample++ ) {
        memcpy ( cvc_loop_stoch_cum[imom][mu][isample],  cvc_loop_stoch_cum[imom][mu][isample-1], T_global * sizeof(double _Complex) ); 
        complex_field_pl_eq_complex_field ( (double*)cvc_loop_stoch_cum[imom][mu][isample], (double*)cvc_loop_stoch_aux[imom][mu][isample], T_global );
      }
    }
  }
#if 0
  ...
  for ( int imom = 0; imom < g_sink_momentum_number; imom++ ) {
    ...
    if ( imom2 == g_sink_momentum_number ) {
      fprintf( stdout, "[loops_hvp_analysis] Error, could not find negative p-vector in momentum list\n");
      EXIT(2);
    }

    fprintf (stdout, "# [loops_hvp_analysis] using imom = %d and imom2 = %d\n", imom, imom2 );
    ...
    if ( ofs == NULL ) {
      fprintf ( stderr, "[loops_hvp_analysis] Error, could not open file %s for writing\n", filename );
      EXIT(1);
    }
  ...
  if(g_cart_id==0) {
    g_the_time = time(NULL);
    fprintf(stdout, "# [loops_hvp_analysis] %s# [loops_hvp_analysis] end of run\n", ctime(&g_the_time));
    fprintf(stderr, "# [loops_hvp_analysis] %s# [loops_hvp_analysis] end of run\n", ctime(&g_the_time));
  }
