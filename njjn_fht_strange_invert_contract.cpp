/***************************************************************************
 *
 * njjn_fht_strange_invert_contract
 *
 ***************************************************************************/
...
int main(int argc, char **argv) {
  ...
  const char flavor_tag[6] = { 'u', 'd', 's', 'S' , 'c', 'C' };
  ...
  */
  ...
  while ((c = getopt(argc, argv, "sSrwch?f:o:g:m:")) != -1) {
    switch (c) {
    case 'f':
    ...
    case 'S':
      ...
      break;
    case 'o':
      loop_op_id = atoi ( optarg );
      break;
    case 'g':
      loop_g5d_sym = atoi ( optarg );
      break;
    case 'm':
      g_mus = atof ( optarg );
      break;
  ...
  fprintf(stdout, "# [njjn_fht_strange_invert_contract] calling tmLQCD wrapper init functions\n");
   ...
   ***************************************************************************/
  if ( g_cart_id == 0 ) {
    fprintf(stdout, "# [njjn_fht_strange_invert_contract] git version = %s\n", g_gitversion);
  }
   ...
   ***************************************************************************/
#ifdef HAVE_OPENMP
  if(g_cart_id == 0) fprintf(stdout, "# [njjn_fht_strange_invert_contract] setting omp number of threads to %d\n", g_num_threads);
  omp_set_num_threads(g_num_threads);
#pragma omp parallel
{
  fprintf(stdout, "# [njjn_fht_strange_invert_contract] proc%.4d thread%.4d using %d threads\n", g_cart_id, omp_get_thread_num(), omp_get_num_threads());
}
#else
  if(g_cart_id == 0) fprintf(stdout, "[njjn_fht_strange_invert_contract] Warning, resetting global thread number to 1\n");
  g_num_threads = 1;
#endif

  if ( init_geometry() != 0 ) {
    fprintf(stderr, "[njjn_fht_strange_invert_contract] Error from init_geometry %s %d\n", __FILE__, __LINE__);
    EXIT(4);
  }
  ...
  if(!(strcmp(gaugefilename_prefix,"identity")==0)) {
    ...
    if(g_cart_id==0) fprintf(stdout, "# [njjn_fht_strange_invert_contract] reading gauge field from file %s\n", filename);
    exitstatus = read_lime_gauge_field_doubleprec(filename);
  } else {
    /* initialize unit matrices */
    if(g_cart_id==0) fprintf(stdout, "\n# [njjn_fht_strange_invert_contract] initializing unit matrices\n");
    exitstatus = unit_gauge_field ( g_gauge_field, VOLUME );
  }
#else
  Nconf = g_tmLQCD_lat.nstore;
  if(g_cart_id== 0) fprintf(stdout, "[njjn_fht_strange_invert_contract] Nconf = %d\n", Nconf);
  ...
  if( g_gauge_field == NULL) {
    fprintf(stderr, "[njjn_fht_strange_invert_contract] Error, g_gauge_field is NULL %s %d\n", __FILE__, __LINE__);
    EXIT(7);
  }
#endif
  ...
  if(exitstatus != 0) {
    fprintf(stderr, "[njjn_fht_strange_invert_contract] Error from gauge_field_eq_gauge_field_ti_phase, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
    EXIT(38);
  }
 ...
 /***************************************************************************
  ...
  if ( exitstatus != 0 ) {
    fprintf(stderr, "[njjn_fht_strange_invert_contract] Error from init_clover, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
    EXIT(1);
  }
  ...
  double **mzz[4]    = { lmzz[0]   , lmzz[1]   , smzz[0]   , smzz[1]    };
  double **mzzinv[4] = { lmzzinv[0], lmzzinv[1], smzzinv[0], smzzinv[1] };
  ...
  io_proc = get_io_proc ();
  if( io_proc < 0 ) {
    fprintf(stderr, "[njjn_fht_strange_invert_contract] Error, io proc must be ge 0 %s %d\n", __FILE__, __LINE__);
    EXIT(14);
  }
  fprintf(stdout, "# [njjn_fht_strange_invert_contract] proc%.4d has io proc id %d\n", g_cart_id, io_proc );
  ...
  gamma_print ( &gammafive, "g5", stdout );
  ...
  if ( g_seq_source_momentum_number > 0 ) {
    ...
    if ( ephase == NULL ) {
      fprintf ( stderr, "[njjn_fht_strange_invert_contract] Error from init_2level_ztable %s %d\n", __FILE__, __LINE__ );
      EXIT(12);
    }
  ...
  if ( exitstatus != 0 ) {
    fprintf(stderr, "[njjn_fht_strange_invert_contract] Error from init_rng_stat_file %s %d\n", __FILE__, __LINE__ );;
    EXIT( 50 );
  }
  ...
  if ( g_coherent_source_number * g_nsample_oet > 0 ) {
    ...
    if( scalar_field == NULL ) {
      fprintf(stderr, "[njjn_fht_strange_invert_contract] Error from init_Xlevel_dtable %s %d\n", __FILE__, __LINE__);
      EXIT(132);
    }
  }
  ...
  if ( ! read_scalar_field  && ( g_coherent_source_number * g_nsample_oet > 0 ) ) {
    ...
    if ( write_scalar_field ) {
      ...
      for ( int i = 0; i < g_coherent_source_number * g_nsample_oet; i++ ) {
        ...
        if ( exitstatus != 0  ) {
          fprintf ( stderr, "[njjn_fht_strange_invert_contract] Error write_lime_contraction, status was %d  %s %d\n", exitstatus, __FILE__, __LINE__ );
          EXIT(12);
        }
      }
    ...
    for ( int i = 0; i < g_coherent_source_number * g_nsample_oet; i++ ) {
      ...
      if ( exitstatus != 0  ) {
        fprintf ( stderr, "[njjn_fht_strange_invert_contract] Error read_lime_contraction, status was %d  %s %d\n", exitstatus, __FILE__, __LINE__ );
        EXIT(12);
      }
    }
  ...
  if ( loop  == NULL ) {
    fprintf ( stderr, "[njjn_fht_strange_invert_contract] Error from init_Xlevel_ztable %s %d\n", __FILE__, __LINE__ );
    EXIT(12);
  }
  ...
  if ( ! read_loop_field ) {
    ...
    if ( g_cart_id == 0 ) {
      fprintf ( stdout, "# [njjn_fht_strange_invert_contract] produce loop field %s %d\n",  __FILE__, __LINE__ );
    }
    ...
    for ( int isample = 0; isample < g_nsample_oet; isample++ ) {
      ...
      for ( int timeslice = 1; timeslice < 3; timeslice ++ )
      {
#endif
        ...
        for ( int ispin = 0; ispin < 2; ispin++ ) 
        {
          ...
          for ( int icol = 0; icol < 2; icol++ ) 
          {
            ...
            if ( timeslice / T == g_proc_coords[0] ) {
              if ( g_verbose > 2 ) fprintf( stdout, "# [njjn_fht_strange_invert_contract] proc %d has global timeslice %d %s %d\n",
...
#pragma omp parallel for
            ...
            if ( g_write_source ) {
              ...
              if ( ( exitstatus = write_propagator ( spinor_work[0], filename, 0, g_propagator_precision) ) != 0 ) {
                fprintf(stderr, "[njjn_fht_strange_invert_contract] Error from write_propagator, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
                EXIT(2);
              }
...
#pragma omp parallel for
            ...
            exitstatus = _TMLQCD_INVERT ( spinor_work[1], spinor_work[0], loop_op_id );
...
#  endif
            {
              fprintf(stderr, "[njjn_fht_strange_invert_contract] Error from invert, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
              EXIT(12);
            }


            if ( check_propagator_residual ) {
              check_residual_clover ( &(spinor_work[1]), &(spinor_work[2]), gauge_field_with_phase, mzz[loop_op_id], 1 );
            }
...
#if _USE_TIME_DILUTION
            if ( timeslice / T == g_proc_coords[0] ) {
              if ( g_verbose > 2 ) fprintf( stdout, "# [njjn_fht_strange_invert_contract] proc %d copy propagator for global timeslice %d %s %d\n", 
                  g_cart_id, timeslice, __FILE__, __LINE__ );
...
#pragma omp parallel for
      ...
      show_time ( &ta, &tb, "njjn_fht_strange_invert_contract", "loop-invert-contract-sample", g_cart_id == 0 );
...
#pragma omp parallel for
    ...
    /***************************************************************************
     * if need be make it g5herm symmetric
     ***************************************************************************/
    if ( loop_g5d_sym == 1 ) {
...
// #pragma omp for
      for ( size_t ix = 0; ix < VOLUME; ix++ )
      {
        ...
        memcpy ( _laux[0], _loop[0], 144 * sizeof( double _Complex ) );

        /* TEST */
        pm_print ( _loop, "S", stdout );
        /* END OF TEST */

        pm_pl_eq_gamma_ti_pm_dag_ti_gamma ( _laux , &gammafive, _loop, 1. );

        pm_eq_pm_ti_co  ( _loop, _laux, 0.5 );

        /* TEST */
        pm_print ( _loop, "Ssym", stdout );
        /* END OF TEST */
     ...
     ***************************************************************************/
    sprintf( filename, "loop.%s.c%d.N%d.lime", flavor_tag[loop_op_id], Nconf, g_nsample );

    if ( io_proc == 2 && g_verbose > 0 ) {
      fprintf ( stdout, "# [njjn_fht_strange_invert_contract] reading loop field from file %s %s %d\n", filename,  __FILE__, __LINE__ );
    }
...
#  endif
  ...
  if( N_Jacobi > 0 ) {
    ...
    if ( N_ape > 0 ) {
      ...
      if(exitstatus != 0) {
        fprintf(stderr, "[njjn_fht_strange_invert_contract] Error from APE_Smearing, status was %d\n", exitstatus);
        EXIT(47);
      }
#ifndef _SMEAR_QUDA 
    }  /* end of if N_ape > 0 */
  ...
  for( int isource_location = 0; isource_location < g_source_location_number; isource_location++ ) {
    ...
    if( exitstatus != 0 ) {
      fprintf(stderr, "[njjn_fht_strange_invert_contract] Error from get_point_source_info status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
      EXIT(123);
    }
     ...
     ***************************************************************************/
      ...
      fprintf(stdout, "# [njjn_fht_strange_invert_contract] writing data to file %s\n", filename);
      affw = aff_writer(filename);
      const char * aff_status_str = aff_writer_errstr ( affw );
      if( aff_status_str != NULL ) {
        fprintf(stderr, "[njjn_fht_strange_invert_contract] Error from aff_writer, status was %s %s %d\n", aff_status_str, __FILE__, __LINE__);
        EXIT(15);
      }
#else
      fprintf(stderr, "[njjn_fht_strange_invert_contract] Error, no outupt variant selected %s %d\n",  __FILE__, __LINE__);
      EXIT(15);
#endif
    }  /* end of if io_proc == 2 */
    ...
    for ( int icoh = 0; icoh < g_coherent_source_number; icoh++ ) {
      ...
      for ( int iflavor = 0; iflavor < 2; iflavor++ ) 
      {
        ...
        if(exitstatus != 0) {
          fprintf(stderr, "[njjn_fht_strange_invert_contract] Error from point_source_propagator, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
          EXIT(12);
        }
      
        gettimeofday ( &tb, (struct timezone *)NULL );
        show_time ( &ta, &tb, "njjn_fht_strange_invert_contract", "forward-light-smear-invert-check", g_cart_id == 0 );
        ...
        for ( int i = 0; i < 12; i++ ) {
          ...
          if(exitstatus != 0) {
            fprintf(stderr, "[njjn_fht_strange_invert_contract] Error from Jacobi_Smearing, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
            return(11);
          }
        }
      
        gettimeofday ( &tb, (struct timezone *)NULL );
        show_time ( &ta, &tb, "njjn_fht_strange_invert_contract", "forward-light-sink-smear", g_cart_id == 0 );
      ...
      for ( int iflavor = 0; iflavor < 2; iflavor++ ) {
        ...
        if ( vx == NULL ) {
          fprintf(stderr, "[njjn_fht_strange_invert_contract] Error from init_2level_dtable, %s %d\n", __FILE__, __LINE__);
          EXIT(47);
        }
        ...
        if ( vp == NULL ) {
          fprintf(stderr, "[njjn_fht_strange_invert_contract] Error from init_3level_dtable %s %d\n", __FILE__, __LINE__ );
          EXIT(47);
        }
        ...
        for ( int if2 = 0; if2 < gamma_f1_number; if2++ ) {
          ...
          if ( exitstatus != 0 ) {
            fprintf(stderr, "[njjn_fht_strange_invert_contract] Error from reduce_project_write, status was %d %s %d\n", exitstatus, __FILE__, __LINE__);
            EXIT(48);
          }
        ...
        gettimeofday ( &tb, (struct timezone *)NULL );
        show_time ( &ta, &tb, "njjn_fht_strange_invert_contract", "n1-n2-reduce-project-write", g_cart_id == 0 );
    ...
    for ( int imom = 0; imom < g_seq_source_momentum_number; imom++ ) {
      ...
      if ( g_cart_id == 0 && g_verbose > 2 ) fprintf(stdout, "# [njjn_fht_strange_invert_contract] start seq source mom %3d %3d %3d  %s %d\n", 
      ...
      for ( int iflavor = 0; iflavor < 2; iflavor++ )
      {
        ...
        if ( g_cart_id == 0 && g_verbose > 2 ) fprintf(stdout, "# [njjn_fht_strange_invert_contract] start seq source flavor %3d   %s %d\n", 
        ...
        for ( int seq_source_type = 0; seq_source_type <= 1; seq_source_type++ )
        {
          ...
          if ( g_cart_id == 0 && g_verbose > 2 ) fprintf(stdout, "# [njjn_fht_strange_invert_contract] start seq source type %3d   %s %d\n", 
          ...
          for ( int igamma = 0; igamma < sequential_gamma_sets; igamma++ ) {
            ...
            if ( g_cart_id == 0 && g_verbose > 2 ) fprintf(stdout, "# [njjn_fht_strange_invert_contract] start seq source gamma set %s   %s %d\n", 
            ...
            for ( int iloop_flavor = 0; iloop_flavor < ( seq_source_type == 0 ? 2 : 1 ); iloop_flavor++ ) {
              ...
              if ( g_cart_id == 0 && g_verbose > 2 ) fprintf(stdout, "# [njjn_fht_strange_invert_contract] using flavor %d / loop_flavor %d for seq source type %d  %s %d\n", 
              ...
              if ( exitstatus != 0 ) {
                fprintf ( stderr, "[njjn_fht_strange_invert_contract] Error from prepare_sequential_fht_loop_source, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
                EXIT(123);
              }
              ...
              gettimeofday ( &tb, (struct timezone *)NULL );
              show_time ( &ta, &tb, "njjn_fht_strange_invert_contract", "prepare-sequential-fht-source", g_cart_id == 0 );
	      ...
	      gettimeofday ( &ta, (struct timezone *)NULL );
              ...
              if ( exitstatus != 0 ) {
                fprintf ( stderr, "[njjn_fht_strange_invert_contract] Error from prepare_propagator_from_source, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
                EXIT(123);
              }
              ...
              gettimeofday ( &tb, (struct timezone *)NULL );
              show_time ( &ta, &tb, "njjn_fht_strange_invert_contract", "sequential-source-invert-check-smear", g_cart_id == 0 );
  	        ...
  	        gettimeofday ( &ta, (struct timezone *)NULL );
                ...
                gettimeofday ( &tb, (struct timezone *)NULL );
                show_time ( &ta, &tb, "njjn_fht_strange_invert_contract", "buuu-duuu-reduce-project-write", g_cart_id == 0 );
                ...
                gettimeofday ( &tb, (struct timezone *)NULL );
                show_time ( &ta, &tb, "njjn_fht_strange_invert_contract", "bddd-dddd-reduce-project-write", g_cart_id == 0 );
	...
	fini_2level_dtable ( &vx );
      ...
      for ( int isample = 0; isample < g_nsample_oet; isample++ ) {
            ...
            for ( int ig = 0; ig < sequential_gamma_num[igamma]; ig++ ) {
              ...
              for ( int iflavor = 0; iflavor < 2; iflavor++ ) 
              {
                ...
                for ( int icoh = 0; icoh < g_coherent_source_number; icoh++)
                {
                  ...
                  if ( exitstatus != 0 ) {
                    fprintf ( stderr, "[njjn_fht_strange_invert_contract] Error from prepare_sequential_fht_twinpeak_source, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
                    EXIT(123);
                  }
		...
		gettimeofday ( &tb, (struct timezone *)NULL );
                show_time ( &ta, &tb, "njjn_fht_strange_invert_contract", "prepare-sequential-fht-w-source", g_cart_id == 0 );
		...
		gettimeofday ( &tb, (struct timezone *)NULL );
                show_time ( &ta, &tb, "njjn_fht_strange_invert_contract", "sequential-source-w-invert-check-smear", g_cart_id == 0 );
	          ...
	          gettimeofday ( &tb, (struct timezone *)NULL );
                  show_time ( &ta, &tb, "njjn_fht_strange_invert_contract", "w-uuuu-diagram-reduce-project-write", g_cart_id == 0 );
	          ...
	          gettimeofday ( &tb, (struct timezone *)NULL );
                  show_time ( &ta, &tb, "njjn_fht_strange_invert_contract", "w-uudd-diagram-reduce-project-write", g_cart_id == 0 );
    ...
    if(io_proc == 2) {
      ...
      if( aff_status_str != NULL ) {
        fprintf(stderr, "[njjn_fht_strange_invert_contract] Error from aff_writer_close, status was %s %s %d\n", aff_status_str, __FILE__, __LINE__);
        EXIT(32);
      }
    }  /* end of if io_proc == 2 */
  ...
  ***************************************************************************/
  ...
  show_time ( &start_time, &end_time, "njjn_fht_strange_invert_contract", "runtime", g_cart_id == 0 );
